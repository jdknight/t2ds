...
function writeMissionSkipList(%name, %val, %client)
{
    ...
    {
        %listfile.writeLine("$SkipMission::name[" @ %k @ "] = \"" @
            $SkipMission::name[%k] @ "\";");
    }
    ...
    if (%client !$= "")
        messageClient(%client, 'MsgAdmin',
            '\c3\"%1\"\c2 %2 mission rotation successful.', %name, %val);
...
function writeTypeSkipList(%name, %val, %client)
{
    ...
    {
        %listfile.writeLine("$SkipType::name[" @ %k @ "] = \"" @
            $SkipType::name[%k] @ "\";");
    }
    ...
    if (%client !$= "")
        messageClient(%client, 'MsgAdmin',
            '\c2Game type \c3\"%1\"\c2 %2 successfully.', %name, %val);
