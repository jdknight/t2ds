...
function StationInventory::setPlayersPosition(%data, %obj, %trigger, %colObj)
{
    ...
    if ((VectorLen(%vel) < 22) && (%obj.triggeredBy != %colObj))
    {
        ...
        %colObj.setTransform(getWord(%pos,0) @ " " @ getWord(%pos,1) @ " " @
            getWord(%pos,2) + 0.8 @ " " @ %rot);//center player on object
...
function StationAmmo::onEndSequence(%data, %obj, %thread)
{
    if (%thread == $ActivateThread)
    {
        ...
        if (%obj.inUse $= "Up")
        {
            ...
            %player = %obj.triggeredBy;
            if (%data.doesRepair && !%player.stationRepairing &&
                    %player.getDamageLevel() != 0)
            {
...
function StationVehicle::setPlayersPosition(%data, %obj, %trigger, %colObj)
{
    ...
    if ((VectorLen(%vel) < 22) && (%obj.triggeredBy != %colObj))
    {
        ...
        %colObj.setvelocity("0 0 0");
        %colObj.setTransform(%posXY @ " " @ %posZ + 0.2 @ " " @ "0 0 " @
            %rotZ @ " " @ %angle); //center player on object
        return true;
    }
...
function MobileInvStation::damageObject(%data, %targetObject, %sourceObject,
        %position, %amount, %damageType)
{
    //If vehicle station is hit then apply damage to the vehicle
    %targetObject.getObjectMount().damage(%sourceObject, %position,
        %amount, %damageType);
}
...
function stationTrigger::onEnterTrigger(%data, %obj, %colObj)
{
    ...
    if (Game.stationOnEnterTrigger(%data, %obj, %colObj))
    {
        ...
        if ((%obj.mainObj.team != %colObj.client.team) && (%obj.mainObj.team != 0))
        {
            ...
            messageClient(%colObj.client, 'msgStationDenied',
                '\c2Access Denied -- Wrong team.~wfx/powered/station_denied.wav');
        }
        else if (%obj.disableObj.isDisabled())
        {
            messageClient(%colObj.client, 'msgStationDisabled',
                '\c2Station is disabled.');
        }
        else if (!%obj.mainObj.isPowered())
        {
            messageClient(%colObj.client, 'msgStationNoPower',
                '\c2Station is not powered.');
        }
        else if (%obj.station.notDeployed)
        {
            messageClient(%colObj.client, 'msgStationNotDeployed',
                '\c2Station is not deployed.');
        }
        else if (%obj.station.triggeredBy $= "")
        {
            if (%obj.station.getDataBlock().setPlayersPosition(
                    %obj.station, %obj, %colObj))
            {
                ...
                %colObj.lastWeapon = (%colObj.getMountedImage($WeaponSlot) == 0)
                    ? "" : %colObj.getMountedImage($WeaponSlot).getName().item;
