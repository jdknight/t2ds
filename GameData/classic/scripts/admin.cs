...
function serverCmdStartNewVote(%client, %typeName, %arg1, %arg2, %arg3, %arg4,
        %playerVote)
{
    ...
    // z0dd - ZOD, 4/7/02. Allow SuperAdmins to kick Admins
    if ((!%isAdmin || (%arg1.isAdmin && %client != %arg1)) && 
            !(%typeName $= "VoteKickPlayer" && %client.isSuperAdmin))
    {
        %teamSpecific = false;
        ...
        if (Game.scheduleVote $= "")
        {
            ...
            if (%playerVote)
            {
                ...
                if (!%sameTeam && %teamSpecific && %typeName !$= "VoteAdminPlayer")
                {
                    messageClient(%client, '',
                        "\c2Player votes must be team based.");
                    return;
                }
                ...
                if (%typeName $= "VoteKickPlayer")
                {
                    ...
                    {
                        messageClient(%client, '',
                            '\c2You can not %1 %2, %3 is a Super Admin!',
                            %actionMsg, %arg1.name, %gender);
                        return;
                    }
                        ...
                        for (%idx = 0; %idx < ClientGroup.getCount(); %idx++)
                        {
                            ...
                            if (%cl.team == %client.team && !%cl.isAIControlled())
                            {
                                messageClient(%cl, 'VoteStarted',
                                    '\c2%1 initiated a vote to %2 %3.',
                                    %client.name, %actionMsg, %arg1.name);
                                %clientsVoting++;
                            }
                        }
                    }
            ...
            else if (%typeName $= "VoteChangeMission")
            {
                for (%idx = 0; %idx < ClientGroup.getCount(); %idx++)
                {
                    ...
                    {
                        messageClient(%cl, 'VoteStarted',
                            '\c2%1 initiated a vote to %2 %3 (%4).',
                            %client.name, %actionMsg, %arg1, %arg2);
                        %clientsVoting++;
                    }
                }
            }
                    ...
                    if (%typeName $= "VoteTournamentMode")
                    {
                        ...
                        if (%admin > 0)
                        {
                            for (%idx = 0; %idx < ClientGroup.getCount(); %idx++)
                            {
                                ...
                                {
                                    messageClient(%cl, 'VoteStarted',
                                        '\c2%1 initiated a vote to %2 Tournament Mode (%3).',
                                        %client.name, %actionMsg, %arg1);
                                    %clientsVoting++;
                                }
                            }
                        }
                        ...
                        {
                            messageClient(%client, 'clientMsg',
                                'There must be a server admin to play in Tournament Mode.');
                            return;
                        }
                    }
                    else
            ...
            else
            {
                for (%idx = 0; %idx < ClientGroup.getCount(); %idx++)
                {
                    ...
                    {
                        messageClient(%cl, 'VoteStarted',
                            '\c2%1 initiated a vote to %2.',
                            %client.name, %actionMsg);
                        %clientsVoting++;
                    }
                }
            }
            ...
            Game.scheduleVote = schedule(($Host::VoteTime * 1000), 0,
