...
function FlipFlop::playerTouch(%data, %flipflop, %player)
{
    ...
    Game.capPlayer[Game.offenseTeam] = stripChars(
        getTaggedString(%player.client.name), "\cp\co\c6\c7\c8\c9");

    // Let the observers know:
    messageTeam(0, 'MsgSiegeTouchFlipFlop',
        '\c2%1 captured the %2 base!~wfx/misc/flipflop_taken.wav',
        %player.client.name, $TeamName[%defTeam]);
    // Let the teammates know:
    messageTeam(%player.team, 'MsgSiegeTouchFlipFlop',
        '\c2%1 captured the %2 base!~wfx/misc/flipflop_taken.wav',
        %player.client.name, $TeamName[%defTeam]);
    // Let the other team know:
    %losers = %player.team == 1 ? 2 : 1;
    messageTeam(%losers, 'MsgSiegeTouchFlipFlop',
...
function SiegeGame::allObjectivesCompleted(%game)
{
    ...
    messageAll('MsgSiegeCaptured', '\c2Team %1 captured the base in %2!',
        $teamName[%game.offenseTeam],
        %game.formatTime($teamScore[%game.offenseTeam], true));
...
function SiegeGame::timeLimitReached(%game)
{
    ...
    messageAll('MsgSiegeFailed', '\c2Team %1 failed to capture the base.',
        $teamName[%game.offenseTeam]);
...
function SiegeGame::halftime(%game, %reason)
{
    ...
    if (%game.firstHalf)
    {
        ...
        //send the message
        messageAll('MsgSiegeRolesSwitched', '\c2Team %1 is now on offense.',
            $teamName[%game.offenseTeam], %game.offenseTeam);
            ...
            if (!%client.isAIControlled())
            {
                ...
                if (%client.team == %oldOffenseTeam)
                {
                    if ($teamScore[%oldOffenseTeam] > 0)
                        messageClient(%client, 'MsgSiegeResult', "",
                            '%1 captured the %2 base in %3!',
                            %game.capPlayer[%oldOffenseTeam],
                            $teamName[%game.offenseTeam],
                            %game.formatTime($teamScore[%oldOffenseTeam], true));
                    else
                        messageClient(%client, 'MsgSiegeResult', "",
                            'Your team failed to capture the %1 base.',
                            $teamName[%game.offenseTeam]);
                }
                else if ($teamScore[%oldOffenseTeam] > 0)
                    messageClient(%client, 'MsgSiegeResult', "",
                        '%1 captured your base in %3!',
                        %game.capPlayer[%oldOffenseTeam],
                        %game.formatTime($teamScore[%oldOffenseTeam], true));
                else
                    messageClient(%client, 'MsgSiegeResult', "",
                        'Your team successfully held off team %1!',
                        $teamName[%oldOffenseTeam]);

                // List out the team rosters:
                messageClient(%client, 'MsgSiegeAddLine', "",
                    '<spush><color:00dc00><font:univers condensed:18><clip%%:50>%1</clip><lmargin%%:50><clip%%:50>%2</clip><spop>',
                ...
                for (%line = 0; %line < %max; %line++)
                {
                    ...
                    messageClient(%client, 'MsgSiegeAddLine', "",
                        '<lmargin:0><clip%%:50> %1</clip><lmargin%%:50><clip%%:50> %2</clip>',
                        %plyr1, %plyr2);
                }
                ...
                commandToClient(%client, 'SetHalftimeClock',
                    $Host::Siege::Halftime / 60000);
...
function SiegeGame::sendDebriefing(%game, %client)
{
        ...
        if (%winner == 1)
        {
            if ($teamScore[2] == 0)
                messageClient(%client, 'MsgDebriefResult', "",
                    '<just:center>Team %1 wins!', $TeamName[1]);
            else
            {
                %timeDiffMS = $teamScore[2] - $teamScore[1];
                messageClient(%client, 'MsgDebriefResult', "",
                    '<just:center>Team %1 won by capturing the base %2 faster!',
                    $TeamName[1], %game.formatTime(%timeDiffMS, true));
            }
        }
        else
        {
            if ($teamScore[1] == 0)
                messageClient(%client, 'MsgDebriefResult', "",
                    '<just:center>Team %1 wins!', $TeamName[2]);
            else
            {
                %timeDiffMS = $teamScore[1] - $teamScore[2];
                messageClient(%client, 'MsgDebriefResult', "",
                    '<just:center>Team %1 won by capturing the base %2 faster!',
                    $TeamName[2], %game.formatTime(%timeDiffMS, true));
            }
        }
    }
    else
    ...
    messageClient(%client, 'MsgDebriefAddLine', "",
        '<spush><color:00dc00><font:univers condensed:18>SUMMARY:<spop>');
    ...
    if ($teamScore[%team1] > 0)
    {
        ...
        messageClient(%client, 'MsgDebriefAddLine', "",
            '<bitmap:bullet_2><lmargin:24>%1 captured the %2 base for Team %3 in %4.<lmargin:0>',
            %game.capPlayer[%team1], $TeamName[%team2], $TeamName[%team1], %timeStr);
    }
    else
        messageClient(%client, 'MsgDebriefAddLine', "",
            '<bitmap:bullet_2><lmargin:24>Team %1 failed to capture the base.<lmargin:0>',
            $TeamName[%team1]);
    ...
    if ($teamScore[%team2] > 0)
    {
        ...
        messageClient(%client, 'MsgDebriefAddLine', "",
            '<bitmap:bullet_2><lmargin:24>%1 captured the %2 base for Team %3 in %4.<lmargin:0>',
            %game.capPlayer[%team2], $TeamName[%team1], $TeamName[%team2], %timeStr);
    }
    else
        messageClient(%client, 'MsgDebriefAddLine', "",
            '<bitmap:bullet_2><lmargin:24>Team %1 failed to capture the base.<lmargin:0>',
...
function SiegeGame::updateKillScores(%game, %clVictim, %clKiller,
...
function SiegeGame::genOnRepaired(%game, %obj, %objName)
{
    if (%game.testValidRepair(%obj))
    {
        ...
        messageTeam(%repairman.team, 'msgGenRepaired',
...
function SiegeGame::stationOnRepaired(%game, %obj, %objName)
{
    ...
    {
        %repairman = %obj.repairedBy;
        messageTeam(%repairman.team, 'msgStationRepaired',
...
function SiegeGame::sensorOnRepaired(%game, %obj, %objName)
{
    ...
    {
        %repairman = %obj.repairedBy;
        messageTeam(%repairman.team, 'msgSensorRepaired',
            '\c0%1 repaired the %2 pulse sensor!', %repairman.name, %obj.nameTag);
    }
}
...
function SiegeGame::turretOnRepaired(%game, %obj, %objName)
{
    ...
    {
        %repairman = %obj.repairedBy;
        messageTeam(%repairman.team, 'msgTurretRepaired',
...
function SiegeGame::vStationOnRepaired(%game, %obj, %objName)
{
    if (%game.testValidRepair(%obj))
    {
        %repairman = %obj.repairedBy;
        messageTeam(%repairman.team, 'msgTurretRepaired',
            '\c0%1 repaired the %2 vehicle station!',
            %repairman.name, %obj.nameTag);
    }
}
...
function SiegeGame::updateScoreHud(%game, %client, %tag)
{
    ...
    if (%game.firstHalf)
        messageClient(%client, 'SetScoreHudHeader', "",
            '<just:center>Team %1 has %2 to capture the base.',
            $teamName[%game.offenseTeam], %curTimeLeftStr);
    else
        messageClient(%client, 'SetScoreHudHeader', "",
            '<just:center>Team %1 must capture the base within %2 to win.',
            $teamName[%game.offenseTeam], %curTimeLeftStr);
    ...
    while (true)
    {
        ...
        if (%client.team != 0)
        {
            messageClient(%client, 'SetLineHud', "", %tag, %index,
                '<tab:20,320>\t<spush>%3<clip:200>%1</clip><spop>\t%4<clip:200>%2</clip>',
        ...
        {
            messageClient(%client, 'SetLineHud', "", %tag, %index,
                '<tab:20,320>\t<spush>%3<clip:200><a:gamelink\t%5>%1</a></clip><spop>\t%4<clip:200><a:gamelink\t%6>%2</a></clip>',
                %team1Client.name, %team2Client.name, %col1Style, %col2Style,
