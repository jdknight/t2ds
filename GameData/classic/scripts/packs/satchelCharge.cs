...
function SatchelChargeThrown::onDestroyed(%this, %object, %lastState)
{
    ...
    else
    {
        ...
        RadiusExplosion(%object, %object.getPosition(), %dmgRadius,
            %dmgMod, %expImpulse, %object.sourceObject, %dmgType);
...
function SatchelChargeTossed::checkVelocity(%data, %item)
{
    ...
    if (VectorLen(%item.getVelocity()) < 0.1)
    {
        ...
        activateSatchel(posFromTransform(%item.getTransform()),
            rotFromTransform(%item.getTransform()), %item.sourceObject);
...
function activateSatchel(%pos, %rot, %source)
{
    ...
    schedule(%satchel.getDatablock().armDelay,
        %satchel, "initArmSatchelCharge", %satchel);
    messageClient(%source.client, 'MsgSatchelChargePlaced',
