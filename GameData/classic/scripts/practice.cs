...
function deployTransferPad(%data, %obj)
{
    ...
    if (!%searchResult)
    {
        if (%obj.inv[%data.getName()] > 0)
            messageClient(%obj.client, 'MsgBeaconNoSurface',
                '\c2Cannot place pad. Too far from surface.');

        return 0;
    }
    %test = $TypeMasks::VehicleObjectType |
    ...
    {
        messageClient(%obj.client, 'MsgBeaconNoSurface',
            '\c2You cannot place this item so close to another object.');
        return 0;
    }
    else if (maxSpawnPads(%obj))
    {
        messageClient(%obj.client, 'MsgDeployFailed',
            '\c2You do not have a vacant telepad slot.~wfx/misc/misc.error.wav');
        return 0;
    }
    else
    {
        ...
        {
            messageClient(%obj.client, 'MsgDeployFailed',
                '\c2Surface is too steep for this item.~wfx/misc/misc.error.wav');
            return;
        }
...
function selectPad(%client)
{
    ...
    messageClient(%client, 'MsgPracMode',
        '\c2Transfer Pad selected: \c3%1', %client.transPad);
}
...
function destroyPad(%client)
{
    if (isObject(%client.spawnPad[%client.transPad-1]))
        ...
        messageClient(%client, 'MsgError',
            '\c2Transfer Pad %1 does not exist!~wfx/misc/misc.error.wav',
            %client.transPad);
}
...
function teleportToPad(%client)
{
    ...
    if (isObject(%pad))
    {
        ...
        {
            messageClient(%client, 'MsgError',
                '\c2You cannot teleport with the flag!~wfx/misc/misc.error.wav');
            %player.throwObject(%player.holdingFlag);
        }
        %pos = getWord(%pad.position, 0) @ " " @ getWord(%pad.position, 1) @
            " " @ (getWord(%pad.position, 2)+0.5);
...
function spawnVehAtPad(%client, %blockname)
{
    ...
    if (isObject(%pad))
    {
        ...
        %height = firstWord(%pos) SPC getWord(%pos, 1) SPC
            (getWord(%pos, 2) + 30);
        ...
        {
            messageClient(%client, 'MsgError',
                '\c2Cannot spawn vehicle. Area above pad is blocked.');
            return;
        }
            ...
            if (vehicleCheck(%blockName, %team))
            {
                ...
                if (%veh)
                {
                    ...
                    if (%client.isVehicleTeleportEnabled())
                    {
                        ...
                        {
                            messageClient(%client, 'MsgError',
                                '\c2You cannot teleport into your vehicle with the flag!~wfx/misc/misc.error.wav');
                ...
                messageClient(%client, 'MsgError',
                    '\c2Your team\'s control network has reached its capacity for this vehicle.~wfx/misc/misc.error.wav');
        }
        else
