...
function checkTurretMount(%data, %obj, %slot)
{
    ...
    if (%potTurret != 0)
    {
        ...
        if (%name $= "TurretBaseLarge" || %name $= "MobileTurretBase" ||
                %name $= %otherMountObj) // z0dd - ZOD, 4/20/02. modular MPB turret
        {
            ...
            if (%potTurret.team == %obj.client.team)
            {
                if (%potTurret.getDamageState() !$= "Enabled")
                {
                    ...
                    messageClient(%obj.client, 'MsgBaseDestroyed',
                        "\c2Turret is disabled, cannot mount barrel."); // z0dd - ZOD, 4/20/02. modular MPB turret
                    %obj.setImageTrigger($BackpackSlot, false);
                }
                else
                {
                    // it's a functional turret base on our team! stick the barrel on it
                    messageClient(%obj.client, 'MsgTurretMount',
                        "\c2Mounting barrel pack on turret."); // z0dd - ZOD, 4/20/02. modular MPB turret
            ...
            {
                // whoops, wrong team
                messageClient(%obj.client, 'MsgTryEnemyTurretMount',
                    "\c2Cannot mount a barrel on an enemy turret base!");
                %obj.setImageTrigger($BackpackSlot, false);
            }
        }
        else
        {
            ...
            messageClient(%obj.client, 'MsgNotTurretBase',
                "\c2Can only mount a barrel on a turret base.");
            %obj.setImageTrigger($BackpackSlot, false);
        }
    }
    else
    {
        ...
        messageClient(%obj.client, 'MsgNoTurretBase',
            "\c2No turret within range.");
        %obj.setImageTrigger($BackpackSlot, false);
    }
}
