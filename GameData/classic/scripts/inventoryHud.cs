...
function InventoryScreen::updateHud(%this, %client, %tag)
{
    ...
    for (%y = 0; $InvWeapon[%y] !$= ""; %y++)
    {
        ...
        for (%i = 0; %i < getFieldCount(%client.weaponIndex); %i++)
        {
            ...
            if ($InvWeapon[%y] $= %client.favorites[getField(
                    %client.weaponIndex, %i)] || !%armor.max[%WInv])
            {
                %notFound = false;
                break;
            }
            else if ("SniperRifle" $= $NameToInv[%client.favorites[getField(
    ...
    if (%noSniperRifle)
    {
        ...
        for (%y = 0; $InvPack[%y] !$= ""; %y++)
        {
            ...
            if ($InvPack[%y] !$= %client.favorites[getField(
                    %client.packIndex, 0)] && %armor.max[%PInv] &&
                    !$InvBanList[%cmt, %PInv])
    ...
    for (%y = 0; $InvGrenade[%y] !$= ""; %y++)
    {
        ...
        for (%i = 0; %i < getFieldCount(%client.grenadeIndex); %i++)
        {
            ...
            if (($InvGrenade[%y] $= %client.favorites[getField(
                    %client.grenadeIndex, %i)]) || !%armor.max[%GInv])
            {
                %notFound = false;
                break;
            }
    ...
    for (%y = 0; $InvMine[%y] !$= "" ; %y++)
    {
        ...
        for (%i = 0; %i < getFieldCount(%client.mineIndex); %i++)
        {
            ...
            if (($InvMine[%y] $= %client.favorites[getField(
                    %client.mineIndex, %i)]) || !%armor.max[%MInv])
            {
                %notFound = false;
                break;
            }
    ...
    %client.numFavsCount++;
    messageClient(%client, 'SetLineHud', "", %tag, 0,
        "Armor:", %armorList, armor, %client.numFavsCount);
    %lineCount = 1;
    ...
    for (%x = 0; %x < %armor.maxWeapons; %x++)
    {
        ...
        messageClient(%client, 'SetLineHud', "", %tag, %x + %lineCount,
            "Weapon Slot " @ %x + 1 @ ": ", %list, weapon, %client.numFavsCount);
    }
    ...
    messageClient(%client, 'SetLineHud', "", %tag, %lineCount,
        "Pack:", %packText, pack, %client.numFavsCount, %packOverFlow);
    %lineCount++;
    ...
    for (%x = 0; %x < %armor.maxGrenades; %x++)
    {
        ...
        messageClient(%client, 'SetLineHud', "", %tag, %x + %lineCount,
            "Grenade:", %list, grenade, %client.numFavsCount);
    }
    ...
    for (%x = 0; %x < %armor.maxMines; %x++)
    {
        ...
        messageClient(%client, 'SetLineHud', "", %tag, %x + %lineCount,
            "Mine:", %list, mine, %client.numFavsCount);
    }
...
{
    // don't forget -- for many functions, anything done here also needs to
    // be done below in buyDeployableFavorites !!!
    %client.player.clearInventory();
    %client.setWeaponsHudClearAll();
    %cmt = $CurrentMissionType;

    %curArmor = %client.player.getDatablock();
    %curDmgPct = getDamagePercent(%curArmor.maxDamage,
        %client.player.getDamageLevel());
    ...
    // if it's a deployable turret, the limit depends on the number of players
    // (deployables.cs)
    ...
    for (%i = 0; %i < getFieldCount(%client.grenadeIndex); %i++)
    {
        if (!$InvBanList[%cmt, $NameToInv[%client.favorites[getField(
                %client.grenadeIndex, %i)]]])
            %client.player.setInventory(
                $NameToInv[%client.favorites[getField(%client.grenadeIndex, %i)]], 30);
    }

    %client.player.lastGrenade = $NameToInv[%client.favorites[getField(
        %client.grenadeIndex, %i)]];
    ...
    for (%i = 0; %i < getFieldCount(%client.mineIndex); %i++)
    {
        if (!$InvBanList[%cmt, $NameToInv[%client.favorites[getField(
                %client.mineIndex, %i)]]])
            %client.player.setInventory($NameToInv[%client.favorites[getField(
                %client.mineIndex, %i)]], 30);
    }
    // End z0dd - ZOD
    // -------------------------------------------------------------------------
...
function buyDeployableFavorites(%client)
{
    ...
    // players cannot buy deployable station packs from a deployable
    // inventory station
    ...
    // if this pack is a deployable that has a team limit, warn the purchaser
    // if it's a deployable turret, the limit depends on the number of
    // players (deployables.cs)
    if (%packChoice $= "TurretIndoorDeployable" ||
            %packChoice $= "TurretOutdoorDeployable")
