...
function ShapeBaseData::onDestroyed(%data, %obj, %prevstate)
{
    ...
    {
        messageAllExcept(%scorer, %damagingTeam, 'MsgDnDObjDisabled',
            '\c2%1 destroyed your %2 objective!',
...
function StaticShapeData::objectiveInit(%data, %obj)
{
    if (!%data.deployedObject && %obj.team > 0)
    {
        ...
        if (%class $= "Generator" || %class $= "Sensor" ||
                %class $= "TurretBase")
        {
...
{
    %name = getTaggedString(%obj.getDataBlock().targetNameTag) SPC
        getTaggedString(%obj.getDataBlock().targetTypeTag);
    ...
    if (%client.isAdmin)
    {
        ...
        default:
            messageClient(%client, 'MsgAdmin',
                '\c2Incorrect value, 0 disables markers, 1 enables markers.');
        }

        messageAll('MsgAdmin', %msg, %snd,
            %val, %adj, %detail, %name, $CurrentMission);
    }
    else
        messageClient(%client, 'MsgAdmin',
...
function DnDGame::clientMissionDropReady(%game, %client)
{
    ...
    for (%i = 1; %i <= %game.numTeams; %i++)
    {
        messageClient(%client, 'MsgDnDAddTeam', "", %i, %game.getTeamName(%i),
            $teamScore[%i], %game.getScoreLimit(%i), -1, -1);
    }
...
{
    // stop the last owner from collecting a cap bonus
    cancel(%this.pCapThread);
    %this.pCapThread = %game.schedule(%game.TIME_REQ_PLYR_CAP_BONUS,
        "awardScorePlayerFFCap", %cl, %this);
    // stop the old owners from collecting any cap bonus
    cancel(%this.tCapThread);
    %this.tCapThread = %game.schedule(%game.TIME_REQ_TEAM_CAP_BONUS,
        "awardScoreTeamFFCap", %this.team, %this);
}
...
function DnDGame::awardScorePlayerFFCap(%game, %cl, %this)
{
    ...
    %cl.flipFlopsCapped++;
    messageClient(%cl, 'msgFFDef',
        '\c0You received a %1 point bonus for holding the %2.',
        %game.SCORE_PER_PLYR_FLIPFLOP_CAP, %game.cleanWord(%this.name));
    messageTeamExcept(%cl, 'msgFFDef',
        '\c0Teammate %1 received a %2 point bonus for holding the %3',
        %cl.name, %game.SCORE_PER_PLYR_FLIPFLOP_CAP,
        %game.cleanWord(%this.name));
    %game.recalcScore(%cl);
}
...
function DnDGame::checkScoreLimit(%game, %team)
{
    ...
    for (%i = 1; %i <= %game.numTeams; %i++)
        messageAll('MsgDnDTeamScores', "", %i, %game.getTeamName(%i),
            $teamScore[%i], %game.getScoreLimit(%i), -1, -1, -1);
...
function DnDGame::onClientDamaged(%game, %clVictim, %clAttacker, %damageType,
        %implement, %damageLoc)
{
...
function DnDGame::recalcScore(%game, %cl)
{
    ...
    %cl.defenseScore =
        %cl.genDefends          * %game.SCORE_PER_OBJECT_DEFEND          +
...
function DnDGame::updateKillScores(%game, %clVictim, %clKiller, %damageType,
        %implement)
{
    ...
    if (isObject(%implement))
    {
        ...
        if (%implement.getDataBlock().getName() $= "AssaultPlasmaTurret" ||
                %implement.getDataBlock().getName() $= "BomberTurret")
...
function DnDGame::vehicleDestroyed(%game, %vehicle, %destroyer)
{
    ...
    if (%vehicle.team == %destroyer.team) // team kill
    {
        ...
        messageAll('msgVehicleTeamDestroy', '\c0%1 TEAMKILLED %3 %2!',
            %destroyerName, %vehicleType, %pref);
    }
    else // legit kill
    {
        ...
        teamDestroyMessage(%destroyer, 'msgVehDestroyed',
            '\c5%1 destroyed an enemy %2!',
            %destroyerName, %vehicleType); // z0dd - ZOD, 8/20/02. Send teammates a destroy message
        ...
        if (%scorer)
        {
            %value = %game.awardScoreVehicleDestroyed(
                %scorer, %vehicleType, %multiplier, %passengers);
            %game.shareScore(%value);
        }
    }
}

function DnDGame::awardScoreVehicleDestroyed(%game, %client, %vehicleType,
        %mult, %passengers)
{
...
function DnDGame::testObjectDefend(%game, %victimID, %killerID)
{
    InitContainerRadiusSearch(%victimID.plyrPointOfDeath,
        %game.RADIUS_OBJECT_DEFENSE, $TypeMasks::StaticShapeObjectType);
    %objID = containerSearchNext();
    while (%objID != 0)
    {
        if (%objID.scoreValue && !%objID.objectiveCompleted &&
                %objID.team == %killerID.team)
            // found the(a) killer's objective near the victim's point of death
            return true;
        else
            %objID = containerSearchNext();
    }

    // didn't find a qualifying objective within required radius of victims
    // point of death
    return false;
}
...
{
    %killerID.objDefends++;
    messageClient(%killerID, 'msgObjDef',
        '\c0You received a %1 point bonus for defending an objective.',
        %game.SCORE_PER_OBJECT_DEFEND);
    messageTeamExcept(%killerID, 'msgObjDef',
        '\c0Teammate %1 received a %2 point bonus for defending an objective.',
        %killerID.name, %game.SCORE_PER_OBJECT_DEFEND);
...
{
    %cl.tkDestroys++;
    teamDestroyMessage(%cl, 'msgTkDes',
        '\c5Teammate %1 destroyed your team\'s %3 objective!',
        %cl.name, %game.cleanWord(%obj.getDataBlock().targetTypeTag));
...
function DnDGame::staticShapeOnRepaired(%game, %obj, %objName)
{
    if (%game.testValidRepair(%obj))
    {
        ...
        teamRepairMessage(%repairman, %tMsgType,
            %tMsg, %repairman.name, %obj.nameTag);
