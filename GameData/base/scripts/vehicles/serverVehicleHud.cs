        ...
        if (%station.ready && %station.station.vehicle[%blockName])
        {
            ...
            for (%x = 0; (%obj = containerSearchNext()) != 0; %x++)
            {
                if ((%obj.getType() & $TypeMasks::VehicleObjectType) &&
                        (%obj.getDataBlock().checkIfPlayersMounted(%obj)))
                {
                    %clear = 0;
                    break;
                }
            ...
            if (%clear)
            {
                ...
                for (%i = 0; %i < %x; %i++)
                {
                    if (%removeObjects[%i].getType() & $TypeMasks::PlayerObjectType)
                    {
                        ...
                        %pData.damageObject(%removeObjects[%i], 0, "0 0 0",
                            1000, $DamageType::VehicleSpawn);
                    }
                    else
                    {
                ...
                schedule(%fadeTime, 0, "createVehicle", %client, %station,
                    %blockName, %team , %p, %rot, %angle);
            }
            else
                messageClient(%client, "",
                    'Can\'t create vehicle. A player is on the creation pad.');
        }
    }
}
...
function createVehicle(%client, %station, %blockName, %team , %pos, %rot, %angle)
{
    ...
    if (%obj)
    {
        ...
        if (%client.isVehicleTeleportEnabled())
            %obj.getDataBlock().schedule(5000,
                "mountDriver", %obj, %client.player);
    }
    if (%obj.getTarget() != -1)
        setTargetSensorGroup(%obj.getTarget(), %client.getSensorGroup());
    // We are now closing the vehicle hud when you buy a vehicle, making the
    // following call unnecessary (and it breaks stuff, too!)
...
function VehicleHud::updateHud(%obj, %client, %tag)
{
    ...
    // Not sure if we need to clear the huds... They'll always have the same
    // num rows...
    ...
    {
        messageClient(%client, 'SetLineHud', "", %tag, %count,
            "GRAV CYCLE", "", ScoutVehicle,
            $VehicleMax[ScoutVehicle] - $VehicleTotalCount[%team, ScoutVehicle]);
    ...
    {
        messageClient(%client, 'SetLineHud', "", %tag, %count,
            "ASSAULT TANK", "", AssaultVehicle,
            $VehicleMax[AssaultVehicle] - $VehicleTotalCount[%team, AssaultVehicle]);
    ...
    {
        messageClient(%client, 'SetLineHud', "", %tag, %count,
            "MOBILE POINT BASE", "", MobileBaseVehicle,
            $VehicleMax[MobileBaseVehicle] - $VehicleTotalCount[%team, MobileBaseVehicle]);
    ...
    {
        messageClient(%client, 'SetLineHud', "", %tag, %count,
            "SCOUT FLIER", "", ScoutFlyer,
            $VehicleMax[ScoutFlyer] - $VehicleTotalCount[%team, ScoutFlyer]);
    ...
    if (%station.vehicle[bomberFlyer])
    {
        messageClient(%client, 'SetLineHud', "", %tag, %count,
            "BOMBER", "", BomberFlyer,
            $VehicleMax[BomberFlyer] - $VehicleTotalCount[%team, BomberFlyer]);
    ...
    if (%station.vehicle[hapcFlyer])
    {
        messageClient(%client, 'SetLineHud', "", %tag, %count,
            "TRANSPORT", "", HAPCFlyer,
            $VehicleMax[HAPCFlyer] - $VehicleTotalCount[%team, HAPCFlyer]);
