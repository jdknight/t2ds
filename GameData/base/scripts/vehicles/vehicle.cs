// Notes:
// - respawning vehicles with turrets (bomber/tank) will not setup the
//   turret properly
...
function VehicleData::onAdd(%data, %obj)
{
    ...
    {
        if ($countDownStarted)
            %data.schedule(($Host::WarmupTime * 1000) / 2,
                "vehicleDeploy", %obj, 0, 1);
        else
        {
...
function VehicleData::onDamage(%this, %obj)
{
    ...
    if (%damage >= %this.destroyedLevel)
    {
        ...
        {
            if (%obj.respawnTime !$= "")
                %obj.marker.schedule = %obj.marker.data.schedule(
                    %obj.respawnTime, "respawn", %obj.marker);
...
function VehicleData::onDestroyed(%data, %obj, %prevState)
{
    ...
    // with it.
    if (%data.getName() $= "BomberFlyer" ||
            %data.getName() $= "MobileBaseVehicle" ||
            %data.getName() $= "AssaultVehicle")
    {
...
function VehicleData::onAvoidCollisions(%data, %obj)
{
    ...
    %position = getWord(%transform, 0) SPC getWord(%transform, 1) SPC
        (getWord(%transform, 2) + 2000);
    %rotation = getWord(%transform, 3) SPC getWord(%transform, 4) SPC
        getWord(%transform, 5) SPC getWord(%transform, 6);
...
function radiusVehicleExplosion(%data, %vehicle)
{
    ...
    InitContainerRadiusSearch(%position, %data.explosionRadius,
    ...
    for (%i = 0; %i < %numTargets; %i++)
    {
        ...
        %amount = (1.0 - (%dist / %data.explosionRadius)) *
            %coverage * %data.explosionDamage;
        %targetData = %targetObject.getDataBlock();

        %momVec = "0 0 1";

        if (%amount > 0)
            %targetData.damageObject(%targetObject, %sourceObject, %position,
                %amount, $DamageType::Explosion, %momVec);
    }
}
...
function BomberFlyer::onAdd(%this, %obj)
{
    ...
    // for this particular weapon - a non-firing datablock used only so the AI
    // can aim the turret; Also needed so we can set the turret parameters..
...
function BomberFlyer::playerMounted(%data, %obj, %player, %node)
{
    ...
    for (%i = 0; %i < %data.numMountPoints; %i++)
        if (%obj.getMountNodeObject(%i) > 0)
            commandToClient(%obj.getMountNodeObject(%i).client,
                'checkPassengers', %passString);
...
function MobileBaseVehicle::checkDeploy(%data, %obj)
{
    ...
    for (%x = 1; %x < 7; %x += 2)
    {
        ...
        %rayCastObj = containerRayCast(%startPos, %endPos,
            $TypeMasks::TerrainObjectType, 0);
        if (%rayCastObj)
...
function MobileBaseVehicle::checkTurretDistance(%data, %obj)
{
    ...
    InitContainerRadiusSearch(%pos, 100, $TypeMasks::TurretObjectType |
        $TypeMasks::InteriorObjectType);
    while ((%objFound = ContainerSearchNext()) != 0)
    {
...
function vehicleListAdd(%blockName, %obj)
{
    for ($i = 0; %i < $VehicleMax[%blockName]; %i++)
    {
        if ($VehicleInField[%obj.team, %blockName, %i] $= "" ||
                $VehicleInField[%obj.team, %blockName, %i] == 0)
        {
...
function findAIEmptySeat(%vehicle, %player)
{
	...
	for (%i = %num; %i < %dataBlock.numMountPoints; %i++)
	{
        ...
        {
            // cheap hack - for now, AI's will mount the next available node
            // regardless of where they collided
            %node = %i;
            break;
        }
	}
...
function VehicleData::onImpact(%data, %vehicleObject, %collidedObject, %vec, %vecLen)
{
    if (%vecLen > %data.minImpactSpeed)
        %data.damageObject(%vehicleObject, 0,
            VectorAdd(%vec, %vehicleObject.getPosition()),
...
function vehicleAbandonTimeOut(%vehicle)
{
    if (%vehicle.getDatablock().cantAbandon $= "" && %vehicle.lastPilot $= "")
    {
        for (%i = 0; %i < %vehicle.getDatablock().numMountPoints; %i++)
            if (%vehicle.getMountNodeObject(%i))
            {
                ...
                if (%passenger.lastVehicle !$= "")
                    schedule(15000, %passenger.lastVehicle,
                        "vehicleAbandonTimeOut", %passenger.lastVehicle);
        ...
        if (%vehicle.respawnTime !$= "")
            %vehicle.marker.schedule = %vehicle.marker.data.schedule(
                %vehicle.respawnTime, "respawn", %vehicle.marker);
        %vehicle.mountable = 0;
...
function VehicleData::respawn(%data, %marker)
{
    %mask = $TypeMasks::PlayerObjectType | $TypeMasks::VehicleObjectType |
        $TypeMasks::TurretObjectType;
    InitContainerRadiusSearch(%marker.getWorldBoxCenter(),
        %data.checkRadius, %mask);
