...
function Bonus::evaluate(%this, %obj1, %obj2, %obj3, %obj4, %obj5)
{
    ...
    {
        %newCategoryData = %this.category[%i].evaluate(
            %obj1, %obj2, %obj3, %obj4);
        if (%newCategoryData > 0)
        {
            ...
            %this.category[%i].schedule(%delay, "performEffects",
                %newCategoryData, %obj2);
        }
    }
...
function Bonus::award(%this, %bonusData, %player1, %player2, %action, %suffix,
        %extraval, %sound)
{
    // Handle instant bonuses (previously handled via subclassing)
    if (%this.instant)
        return %this.awardInstantBonus(%bonusData, %player1, %player2, %action,
            %suffix, %extraval, %sound);
...
function Bonus::awardInstantBonus(%this, %bonusData, %player1, %player2,
        %action, %suffix, %extraVal, %sound)
{
    if (%bonusData !$= "")
        %val = %bonusData.getValue();
    else
    ...
    //messageAll('MsgTR2Event', "", %summary, %text, %val);
    messageTeam(%scoringTeam, 'MsgTR2InstantBonus',
        "\c4" @ %text SPC "\c3(+" @ %val @ ")");
    messageTeam(%otherTeam, 'MsgTR2InstantBonus',
        "\c4" @ %text SPC "\c3(+" @ %val @ ")");
    messageTeam(0, 'MsgTR2InstantBonus',
        "\c4" @ %text SPC "\c3(+" @ %val @ ")");
...
function WeaponBonus::award(%this, %bonusData, %shooter, %victim)
{
    ...
    Parent::award(%this, %bonusData, %shooter, %victim, "immobilized",
        "Hit \c2(" @ getTaggedString(%shooter.client.name) @ ")");
}
...
{
    Parent::award(%this, %bonusData, %plPasser, %plReceiver, %action,
        %desc, %val);
}
