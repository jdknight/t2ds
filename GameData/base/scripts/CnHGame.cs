...
function CnHGame::gameOver(%game)
{
    ...
    {
        $TeamScore[%team] = 0;
        messageAll('MsgCnHTeamCap', "", -1, -1, -1,
            %team, $TeamScore[%team], %game.getScoreLimit());
    }
}
...
function CnHGame::clientMissionDropReady(%game, %client)
{
    ...
    for (%i = 1; %i <= %game.numTeams; %i++)
    {
        ...
        messageClient(%client, 'MsgCnHAddTeam', "", %i,
            $TeamName[%i], $TeamScore[%i], %scoreLimit, %teamHeld);
    }
...
function CnHGame::testPlayerFFDefend(%game, %victimID, %killerID)
{
    ...
    // loop through the flipflops looking for one within range that belongs to
    // the killer...
    %ffGroup = nameToID("MissionCleanup/FlipFlops");
    for (%i = 0; %i < %ffGroup.getCount(); %i++)
    {
...
function CnHGame::awardScorePlayerFFDefend(%game, %cl, %flipflop)
{
    ...
    if (%game.SCORE_PER_FLIPFLOP_DEFEND != 0)
    {
        messageClient(%cl, 'msgFFDef',
            '\c0You received a %1 point bonus for defending %2.',
            %game.SCORE_PER_FLIPFLOP_DEFEND, %game.cleanWord(%flipflop.name));
        //messageTeamExcept(%cl, 'msgFFDef',
        //  '\c0Teammate %1 received a %2 point bonus for defending %3',
        //  %cl.name, %game.SCORE_PER_FLIPFLOP_DEFEND, %game.cleanWord(%flipflop.name));
    }
...
function CnHGame::awardScorePlayerFFCap(%game, %cl, %this)
{
    ...
    if (%game.SCORE_PER_PLYR_FLIPFLOP_CAP != 0)
    {
        ...
        //  %cl.name, %game.SCORE_PER_PLYR_FLIPFLOP_CAP, %game.cleanWord(%this.name));
    }
...
function CnHGame::awardScoreTeamFFCap(%game, %team, %this)
{
    ...
    if (%game.SCORE_PER_TEAM_FLIPFLOP_HOLD != 0)
        %this.tHoldThread = %game.schedule(%game.TIME_REQ_TEAM_HOLD_BONUS,
            "awardScoreTeamFFHold", %team, %this);
...
function CnHGame::startTimerPlayerFFCap(%game, %cl, %this)
{
    ...
    %this.tCapThread = %game.schedule(%game.TIME_REQ_TEAM_CAP_BONUS,
        "awardScoreTeamFFCap", %cl.team, %this);
}

function CnHGame::startTimerTeamFFCap(%game, %team, %this, %time)
{
    %this.tCapThread = %game.schedule(%time,
        "awardScoreTeamFFCap", %team, %this);
}
