...
function DMGame::setUpTeams(%game)
{
    ...
    while (%team != -1)
    {
        // create drop set and add all spawn sphere objects into it
...
function DMGame::updateKillScores(%game, %clVictim, %clKiller, %damageType, %implement)
{
    ...
    messageClient(%clVictim, 'MsgDMPlayerDies', "",
        %clVictim.deaths + %clVictim.suicides);
}
...
function DMGame::recalcScore(%game, %client)
{
    ...
    if (%killValue - %deathValue == 0)
        ...
        %client.efficiency = ((%killValue * %killValue) /
            (%killValue - %deathValue)) + %suicideValue;
    ...
    for (%index = 0; %index < $TeamRank[0, count]; %index++)
    {
        ...
        if (%client.team != 0)
        {
            messageClient(%client, 'SetLineHud', "", %tag, %index,
                '%5<tab:20, 450>\t<clip:200>%1</clip><rmargin:280><just:right>%2<rmargin:370><just:right>%3<rmargin:460><just:right>%4',
        ...
        {
            messageClient(%client, 'SetLineHud', "", %tag, %index,
                '%5<tab:20, 450>\t<clip:200><a:gamelink\t%6>%1</a></clip><rmargin:280><just:right>%2<rmargin:370><just:right>%3<rmargin:460><just:right>%4',
