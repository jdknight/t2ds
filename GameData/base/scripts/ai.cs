...
function AIInitObjectives(%team, %game)
{
    ...
    for (%i = 0; %i < %count; %i++)
    {
        ...
        //if the objective is not an "AIObjective", assume it's a group and
        // continue
        if (%objective.getClassName() !$= "AIObjective")
            continue;
...
function AIDeployObject(%client, %object)
{
    ...
    if (%type $= "TurretDeployedFloorIndoor" ||
            %type $= "TurretDeployedWallIndoor" ||
            %type $= "TurretDeployedCeilingIndoor" ||
            %type $= "TurretDeployedOutdoor")
    {
...
function AIConnection::onAIDrop(%client)
{
    ...
    //kill the player, which should cause the Game object to perform whatever
    //cleanup is required.
...
function AIConnection::onAIConnect(%client, %name, %team, %skill, %offense,
        %voice, %voicePitch)
{
    // Sex/Race defaults
    %client.sex = "Male";
    ...
    messageAllExcept(%client, -1, 'MsgClientJoin',
        '\c1%1 joined the game.', %name, %client, %client.target, true);
...
function onAIRespawn(%client)
{
    ...
    if (!isEventPending(%client.objectiveThread))
    {
        ...
        if (%schedTime <= 0)
            %schedTime += 5000;
        %client.objectiveThread = schedule(%schedTime, %client,
...
function AIClientIsAlive(%client, %duration)
{
    ...
    if (isObject(%client.player))
    {
        ...
        if (%state !$= "Dead" && %state !$= "" &&
                (%duration $= "" || getSimTime() - %client.spawnTime >= %duration))
            return true;
        else
            return false;
    }
...
function AIFindClosestEnemy(%srcClient, %radius, %losTimeout)
{
    ...
    return AIFindClosestEnemyToLoc(%srcClient, %srcLocation, %radius,
        %losTimeout, false, true);
}

function AIFindClosestEnemyToLoc(%srcClient, %srcLocation, %radius, %losTimeout,
        %ignoreLOS, %distFromClient)
{
    if (%ignoreLOS $= "")
        %ignoreLOS = false;
    if (%distFromClient $= "")
    ...
    for (%i = 0; %i < %count; %i++)
    {
        ...
        if (AIClientIsAlive(%cl) && %cl.team != %srcClient.team)
        {
            ...
            if (%ignoreLOS || %hasLOS || (%losTime < %losTimeout &&
...
function AIFindClosestEnemyPilot(%client, %radius, %losTimeout)
{
    ...
    for (%i = 0; %i < %count; %i++)
    {
        ...
        if (%hasLOS || (%losTime < %losTimeout &&
                AIClientIsAlive(%pilot.client, %losTime + 1000)))
        {
...
function AIFindAIClientInView(%srcClient, %team, %radius)
{
    ...
    %srcEyePoint = firstWord(%srcEyeTransform) @ " " @
        getWord(%srcEyeTransform, 1) @ " " @ getWord(%srcEyeTransform, 2);
    ...
    for (%i = 0; %i < %count; %i++)
    {
        ...
        if (%cl != %srcClient && AIClientIsAlive(%cl) && %cl.isAIControlled() &&
                (%team < 0 || %cl.team == %team))
        {
            ...
            if (%radius <= 0 || %distance <= %radius)
            {
                ...
                //see if the dot product is greater than our current, and
                //greater than 0.6
