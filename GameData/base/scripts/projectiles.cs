...
//-------------------------------------- Default functions
//
function ProjectileData::onCollision(%data, %projectile, %targetObject,
...
function SniperProjectileData::onCollision(%data, %projectile, %targetObject,
        %modifier, %position, %normal)
{
    ...
    %targetObject.damage(%projectile.sourceObject, %position,
        %modifier * %damageAmount, %data.directDamageType);
}
...
function ShapeBaseImageData::onFire(%data, %obj, %slot)
{
    ...
    if (%obj.client > 0)
    {
        // fire your weapon and your invincibility goes away.
...
function MissileBarrelLarge::onFire(%data, %obj, %slot)
{
    ...
    if (%target)
        ...
        // set as unguided. Only happens when itchy trigger can't wait for
        // lock tone.
        %p.setNoTarget();
}
...
function ShockLanceImage::onFire(%this, %obj, %slot)
{
    ...
    %endPos = VectorAdd(%muzzlePos,
        VectorScale(%muzzleVec, %this.projectile.extension));
    ...
    %hit = ContainerRayCast(%muzzlePos, %endPos,
        %damageMasks | %everythingElseMask, %obj);
    ...
    if (%hit !$= "0")
    {
        ...
        %hitpos = getWord(%hit, 1) @ " " @ getWord(%hit, 2) @
            " " @ getWord(%hit, 3);
        ...
        if (%hitObj.getType() & %damageMasks)
        {
            ...
            // This is truly lame, but we need the src obj property present...
            ...
            if (%hitObj.getDataBlock().getClassName() $= "PlayerData")
            {
                ...
                %objDir2D   = getWord(%forwardVec, 0) @ " " @
                    getWord(%forwardVec,1) @ " " @ "0.0";
...
function ELFProjectileData::targetDestroyedCancel(%data, %projectile,
    %target, %targeter)
{
    cancel(%projectile.ELFrecur);
    %target.stopAudio($ELFZapSound);
...
function ELFProjectile::checkELFStatus(%this, %data, %target, %targeter)
{
    if (isObject(%target))
    {
        ...
        if (%enLevel < 1.0)
        {
            ...
            %dataBlock.damageObject(%target, %this.sourceObject,
                %target.getPosition(), %data.drainHealth,
                %data.directDamageType);
        }
        else
        {
            %normal = "0.0 0.0 1.0";
        ...
        %this.ELFrecur = %this.schedule(32, checkELFStatus, %data,
            %target, %targeter);
...
function RadiusExplosion(%explosionSource, %position, %radius, %damage,
        %impulse, %sourceObject, %damageType)
{
    ...
    for (%i = 0; %i < %numTargets; %i++)
    {
        ...
        if (%impulse && %data.shouldApplyImpulse(%targetObject))
        {
            ...
            %impulseVec = VectorScale(%momVec,
                %impulse * (1.0 - (%dist / %radius)));
            %doImpulse = true;
        }
        // ---------------------------------------------------------------------
        // z0dd - ZOD, 5/8/02. Removed Wheeled Vehicle to eliminate the flying
        // MPB bug caused by tossing concussion grenades under a deployed MPB.
        ...
        if (%amount > 0)
            %data.damageObject(%targetObject, %sourceObject, %position,
                %amount, %damageType, %momVec, %explosionSource.theClient,
                %explosionSource);
                ...
                %data.getClassName() $= "PlayerData")
        {
            ...
            if (!$teamDamage && %sourceObject != %targetObject &&
                    %sourceObject.client.team == %targetObject.client.team)
            {
