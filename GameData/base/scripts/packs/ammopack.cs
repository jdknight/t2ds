...
function AmmoPack::onPickup(%this, %pack, %player, %amount)
{
    ...
    // if player doesn't have any grenades at all, give 'em whatever the
    // ammo pack has
    if (!%gFound)
    {
        %given = false;
    ...
    else
    {
        ...
        if (%pack.inv[%playerGrenType] > 0)
        {
            ...
            %player.incInventory(%playerGrenType,
                %pack.getInventory(%playerGrenType));
        ...
        {
            // pack is uninitialized for this grenade type -- meaning it has
            // full count
            %player.incInventory(%playerGrenType, %this.max[%playerGrenType]);
        }
    }
    // now see if player had mines selected and if they're allowed in
    // this mission type
...
function AmmoPack::onInventory(%this, %player, %value)
{
    ...
    // the below test is necessary because this function is called every time
    // the ammo pack gains or loses an item
    if (%player.getClassName() $= "Player")
    {
        if (!%value)
            ...
            {
                // ammo pack was sold at inventory station -- no need to worry
                // about setting the ammo in the pack object (it doesn't exist
                // any more)
...
{
    // %packObj = Ammo Pack object number if pack is being thrown, -1 if sold
    //             at inv station
    ...
    for (%i = 0; %i < $numAmmoItems; %i++)
    {
        ...
        // %pMax == how much of this ammo type the player's datablock says
        // can be carried
        ...
        else
        {
            if (%packObj > 0)
            {
                // the pack gets -1 of this ammo type -- else it'll assume it's
                // full can't use setInventory() because it won't allow values
                // less than 1
                %packObj.inv[%ammo] = -1;
            }
        }
    ...
    for (%j = 0; %j < $numGrenTypes; %j++)
    {
        ...
        {
            // ammo pack only carries type of grenades that player who bought
            // it (or picked it up) had at the time -- all else are zeroed out
            // (value of -1)
            if (%packObj > 0)
                %packObj.inv[%gType] = -1;
        }
    ...
    {
        // player doesn't have any grenades at all. nothing needs to be
        // done here.
    }
    ...
    if (%player.getInventory(Mine) > %player.getDatablock().max[Mine])
    {
        ...
        if (%packObj > 0)
        {
            ...
            %packObj.setInventory(Mine,
                %player.getInventory(Mine) - %player.getDatablock().max[Mine]);
        }
    ...
    else
    {
        if (%packObj > 0)
        {
            // the pack gets -1 for mines -- else it'll assume it's full can't
            // use setInventory() because it won't allow values less than 1
            %packObj.inv[Mine] = -1;
        }
    }
}
