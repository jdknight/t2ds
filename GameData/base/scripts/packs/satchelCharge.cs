...
function SatchelCharge::onUse(%this, %obj)
{
    ...
    schedule(%item.getDatablock().armDelay,
        %item, "initArmSatchelCharge", %item);
    messageClient(%obj.client, 'MsgSatchelChargePlaced',
        "\c2Satchel charge deployed.");
}
...
function SatchelChargeThrown::onDestroyed(%this, %object, %lastState)
{
    ...
    else
    {
        ...
        // the "thwart" flag is set if the charge is destroyed with weapons
        // rather than detonated. A less damaging explosion, but visually the
        // same scale.
        if (%object.thwart)
        {
            messageClient(%object.sourceObject.client,
                'msgSatchelChargeDetonate', "\c2Satchel charge destroyed.");
            %dmgRadius = 10;
            %dmgMod = 0.3;
            %expImpulse = 1000;
        ...
        {
            messageClient(%object.sourceObject.client,
                'msgSatchelChargeDetonate', "\c2Satchel charge detonated!");
            %dmgRadius = 20;
            %dmgMod = 1.0;
            %expImpulse = 2500;
        ...
        RadiusExplosion(%object, %object.getPosition(), %dmgRadius, %dmgMod,
            %expImpulse, %object.sourceObject, %dmgType);
...
function SatchelChargeThrown::damageObject(%data, %targetObject, %sourceObject,
