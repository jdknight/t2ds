...
function CloakingPackImage::onActivate(%data, %obj, %slot)
{
    ...
    {
        // can the player currently cloak (function returns "true" or
        // reason for failure)?
        if (%obj.canCloak() $= "true")
        {
            messageClient(%obj.client, 'MsgCloakingPackOn',
                '\c2Cloaking pack on.');
            %obj.setCloaked(true);
        ...
        else
        {
            ...
            messageClient(%obj.client, 'MsgCloakingPackFailed',
                '\c2Jamming field prevents cloaking.');
    ...
    else
    {
        ...
        messageClient(%obj.client, 'MsgCloakingPackInvalid',
            '\c2Cloaking available for light armors only.');
        %obj.setImageTrigger(%slot, false);
    }
}
...
function CloakingPackImage::onDeactivate(%data, %obj, %slot)
{
    ...
    // if pack is not on then don't bother...
...
function Armor::onForceUncloak(%this, %obj, %reason)
{
    ...
    if (%obj.getImageState($BackpackSlot) $= "activate")
    {
        ...
        messageClient(%obj.client, 'MsgCloakingPackOff',
            '\c2Cloaking pack off.  Jammed.');
        %obj.setCloaked(false);
