...
function RepairPackImage::onUnmount(%data, %obj, %node)
{
    ...
    // if the player was repairing something when the pack was thrown, stop
    // repairing it
    if (%obj.repairing != 0)
        stopRepairing(%obj);
}
...
function RepairGunImage::onValidate(%this, %obj, %slot)
{
    ...
    if (%scanTarg && !(%scanTarg.getType() & $TypeMasks::InteriorObjectType))
    {
        ...
        if (%repTgt.notRepairable)
        {
            ...
            if (!%obj.errMsgSent)
            {
                messageClient(%obj.client, 'MsgRepairPackIrrepairable',
                    '\c2Target is not repairable.', %repTgt);
                %obj.errMsgSent = true;
            }
        ...
        else if (%repTgt.getDamageLevel())
        {
            ...
            // setting imageLoaded to true sends us to repair state
            // (function onRepair)
        ...
        else
        {
            ...
            if (!%obj.errMsgSent)
            {
                // if the target isn't damaged, send a message to that
                // effect only once
                messageClient(%obj.client, 'MsgRepairPackNotDamaged',
                    '\c2Target is not damaged.', %repTgt);
                %obj.errMsgSent = true;
            }
            // if player was repairing something, stop the repairs -- we're done
            if (%obj.repairing)
    ...
    // AI hack - too many things influence the aiming, so I'm going to force the
    // repair object for bots only
    ...
    else if (%obj.getDamageLevel())
    {
        ...
        // check to see if we were repairing something before -- if so, stop
        // repairing old target
        if (%obj.repairing != 0)
    ...
    else
    {
        ...
        if (!%obj.errMsgSent)
        {
            ...
            messageClient(%obj.client, 'MsgRepairPackNoTarget',
                '\c2No target to repair.');
            %obj.errMsgSent = true;
        }
        stopRepairing(%obj);
    }
...
function RepairGunImage::onRepair(%this, %obj, %slot)
{
    ...
    {
        // keep track of who last repaired this item
        %target.repairedBy = %obj.client;
        if (%obj.repairing == %obj)
        {
            ...
            if (%obj.getDamageLevel())
            {
                ...
                {
                    // no need for a projectile, just send a message and up
                    // the repair rate
                    messageClient(%obj.client, 'MsgRepairPackPlayerSelfRepair',
                        '\c2Repairing self.');
            ...
            {
                messageClient(%obj.client, 'MsgRepairPackSelfDone',
                    '\c2Repairs completed on self.');
                stopRepairing(%obj);
                %obj.errMsgSent = true;
            }
        }
        else
        {
            ...
            if (%obj.client.isAIControlled() &&
                    isObject(%obj.client.repairObject) &&
                    %obj.client.repairObject == %obj.repairing)
            {
                %repTgt = %obj.client.repairObject;
                %repPoint = %repTgt.getAIRepairPoint();
                if (%repPoint $= "0 0 0")
                ...
                // if the dot product is very close (i.e. we're aiming in the
                // right direction)
                if (VectorDot(%repTgtVector, %aimVector) > 0.85)
                    %scanTarg = ContainerRayCast(
                        %muzPoint, %repPoint, %searchMasks, %obj);
            }
            else
                %scanTarg = ContainerRayCast(%muzPoint, %rangeEnd, %searchMasks, %obj);
            ...
            if (%scanTarg)
            {
                ...
                %obstructMask = $TypeMasks::InteriorObjectType |
                    $TypeMasks::TerrainObjectType;
                %obstruction = ContainerRayCast(
                    %muzPoint, %pos, %obstructMask, %obj);
                if (%obstruction)
                    %scanTarg = "0";
            }
            ...
            if (%scanTarg)
            {
                ...
                if (%repTgt.getDamageLevel())
                {
                    if (%repTgt != %obj.repairing)
                    {
                        // the target is not the same as the one we were just
                        // repairing stop repairing old target, start repairing
                        // new target
                        stopRepairing(%obj);
                        if (isObject(%obj.repairing))
                            stopRepairing(%obj);
                        ...
                        %obj.repairing = %repTgt;
                        // extract the name of what player is repairing based on
                        // what it is:
                        // - if it's a player, it's the player's name (duh)
                        // - if it's an object, look for a nametag
                        // - if object has no nametag, just say what it is
                        //    (e.g. generatorLarge)
                        ...
                        messageClient(%obj.client, 'MsgRepairPackRepairingObj',
                            '\c2Repairing %1.', %tgtName, %repTgt);
                        startRepairing(%obj, false);
                    }
                    else
                    {
                        // it's the same target as last time, changed to fix
                        // "2 players can't repair same object" bug
                        if (%obj.repairProjectile == 0)
                        {
                            if (%repTgt.getClassName() $= Player)
                                ...
                                %tgtName = %repTgt.getDatablock().getName();
                            messageClient(%obj.client,
                                'MsgRepairPackRepairingObj',
                                '\c2Repairing %1.', %tgtName, %repTgt);
                            startRepairing(%obj, false);
                        }
                    }
                ...
                else
                {
                    ...
                    if (%repTgt != %obj.repairing)
                    {
                        ...
                        messageClient(%obj.client, 'MsgRepairPackNotDamaged',
                            '\c2%1 is not damaged.', %tgtName);
                    }
                    else
                    {
                        // same target, but not damaged -- we must be done
                        messageClient(%obj.client, 'MsgRepairPackDone',
                            '\c2Repairs completed.');
            ...
            else
            {
                ...
                messageClient(%obj.client, 'MsgRepairPackLostTarget',
                    '\c2Repair target no longer in range.');
                stopRepairing(%obj);
            }
        }
    }
...
function stopRepairing(%player)
{
    ...
    else if (%player.repairing > 0)
    {
        ...
        %player.repairing.setRepairRate(
            %player.repairing.getRepairRate() - %player.repairingRate);
        //}
...
function startRepairing(%player, %self)
{
    ...
    if (%self)
    {
        ...
        %player.setRepairRate(
            %player.getRepairRate() + RepairGunImage.repairFactorPlayer);
    ...
    else
    {
        ...
        if (%player.client.isAIControlled() &&
                %player.client.repairObject == %player.repairing)
        {
            %initialPosition  = %player.getMuzzlePoint($WeaponSlot);
            %initialDirection = VectorSub(
                %initialPosition, %player.repairing.getWorldBoxCenter());
        }
        else
        {
        ...
        %player.repairing.setRepairRate(
            %player.repairing.getRepairRate() + %repRate);
