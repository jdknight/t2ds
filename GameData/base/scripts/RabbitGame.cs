...
function RabbitGame::setUpTeams(%game)
{
    ...
    if (!Game.teamMode)
    {
        ...
        // non-rabbits can listen to the rabbit: all others can only listen
        // to self
        setSensorGroupListenMask($NonRabbitTeam,
            (1 << $RabbitTeam) | (1 << $NonRabbitTeam));
    }
}
...
// ----- These functions supersede those in DefaultGame.cs
...
function RabbitGame::AIHasJoined(%game, %client)
{
    ...
    //  '%1 has joined the hunt.', %client.name, "", %client, $NonRabbitTeam);
}
...
function RabbitGame::assignClientTeam(%game, %client)
{
    ...
    messageAll('MsgClientJoinTeam', '\c1%1 has joined the hunt.',
        %client.name, "", %client, %client.team);
...
function RabbitGame::playerSpawned(%game, %player)
{
    ...
    if (isObject(%clRabbit) && !%player.client.isAIControlled())
        %player.client.waypointSchedule = %game.showRabbitWaypointClient(
            %clRabbit, %player.client);
}
...
function RabbitGame::recalcScore(%game, %client)
{
    ...
    %client.score = %client.flagGrabs + %client.kills +
        mFloor(%timeHoldingFlagMS / 15000);
    messageClient(%client, 'MsgYourScoreIs', "", %client.score);
    %game.recalcTeamRanks(%client);
    %game.checkScoreLimit(%client);
}

function RabbitGame::onClientDamaged(%game, %clVictim, %clAttacker,
        %damageType, %sourceObject)
{
    // if the victim is the rabbit, and the attacker is not the rabbit, set
    // the damage time...
...
function RabbitGame::onClientKilled(%game, %clVictim, %clKiller,
...
function RabbitGame::playerDroppedFlag(%game, %player)
{
    ...
    messageAllExcept(%player.client, -1, 'MsgRabbitFlagDropped',
        '\c2%1 dropped the flag!~wfx/misc/flag_drop.wav', %player.client.name);
...
{
    // this function calculates the score for the rabbit. It must be done
    // periodically since the rabbit's score is based on how long the flag has
    // been in possession.
    if (%player.holdingFlag != 0 && %player.getState() !$= "Dead")
    {
        %game.recalcScore(%player.client);
        //reschedule this flag-check for 5 seconds
        %game.schedule(5000, "RabbitFlagCheck", %player);
    }
}
...
function RabbitGame::checkScoreLimit(%game, %client)
{
    ...
    // default of 1200 if scoreLimit not defined
    //  (that's 1200 seconds worth - 20 minutes)
    if (%scoreLimit $= "")
        %scoreLimit = 1200;
...
function RabbitGame::AlertPlayer(%game, %count, %player)
{
    ...
    {
        if (%count > 1)
            %player.alertThread = %game.schedule(
                1000, "AlertPlayer", %count - 1, %player);
        else
            %player.alertThread = %game.schedule(
                1000, "MissionAreaDamage", %player);
    }
    // keep the thread going for non rabbits, but give the new rabbit time
...
function RabbitGame::MissionAreaDamage(%game, %player)
{
    if (%player.getState() !$= "Dead")
    {
        ...
        // a little redundancy to see if the latest damage killed the player
        if (%player.getState() $= "Dead")
...
function RabbitGame::updateScoreHud(%game, %client, %tag)
{
    ...
    if (%numColumns == 2)
        messageClient(%client, 'SetScoreHudSubheader', "",
            '<tab:5,155,225,305,455,525>\tPLAYER\tSCORE\tTIME\tPLAYER\tSCORE\tTIME');
    else
        messageClient(%client, 'SetScoreHudSubheader', "",
            '<tab:15,235,335>\tPLAYER\tSCORE\tTIME');
    ...
    for (%index = 0; %index < %countMax; %index++)
    {
        ...
        {
            %col1ClientTimeMS = %col1Client.flagTimeMS + getSimTime() -
                %col1Client.startTime;
        ...
        if (%numColumns == 2)
        {
            ...
            if (%col2Index < %numClients)
            {
                ...
                {
                    %col2ClientTimeMS = %col2Client.flagTimeMS + getSimTime() -
                        %col2Client.startTime;
        ...
        if (%client.team != 0)
        {
            if (%numColumns == 2)
                messageClient(%client, 'SetLineHud', "", %tag, %index,
                    '<tab:10><spush>%7\t<clip:150>%1</clip><rmargin:205><just:right>%2<rmargin:270><just:right>%3<spop><rmargin:505><lmargin:310>%8<just:left>%4<rmargin:505><just:right>%5<rmargin:570><just:right>%6',
                    %col1Client.name, %col1ClientScore, %col1ClientTime,
                    %col2Client.name, %col2ClientScore, %col2ClientTime,
                    %col1Style, %col2Style);
            else
                messageClient(%client, 'SetLineHud', "", %tag, %index,
                    '<tab:20>%4\t<clip:200>%1</clip><rmargin:280><just:right>%2<rmargin:375><just:right>%3',
                    %col1Client.name, %col1ClientScore, %col1ClientTime,
                    %col1Style);
        }
        ...
        else
        {
            if (%numColumns == 2)
            {
                // this is really crappy, but I need to save 1 tag - can only
                // pass in up to %9, %10 doesn't work...
                if (%col2Style $= "<color:00dc00>")
                {
                    messageClient(%client, 'SetLineHud', "", %tag, %index,
                        '<tab:10><spush>%7\t<clip:150><a:gamelink\t%8>%1</a></clip><rmargin:205><just:right>%2<rmargin:270><just:right>%3<spop><rmargin:505><lmargin:310><color:00dc00><just:left><clip:150><a:gamelink\t%9>%4</a></clip><rmargin:505><just:right>%5<rmargin:570><just:right>%6',
                        %col1Client.name, %col1ClientScore, %col1ClientTime,
                        %col2Client.name, %col2ClientScore, %col2ClientTime,
                ...
                {
                    messageClient(%client, 'SetLineHud', "", %tag, %index,
                        '<tab:10><spush>%7\t<clip:150><a:gamelink\t%8>%1</a></clip><rmargin:205><just:right>%2<rmargin:270><just:right>%3<spop><rmargin:505><lmargin:310><color:dcdcdc><just:left><clip:150><a:gamelink\t%9>%4</a></clip><rmargin:505><just:right>%5<rmargin:570><just:right>%6',
                        %col1Client.name, %col1ClientScore, %col1ClientTime,
                        %col2Client.name, %col2ClientScore, %col2ClientTime,
                        %col1Style, %col1Client, %col2Client);
                }
                ...
                messageClient(%client, 'SetLineHud', "", %tag, %index,
                    '<tab:20>%4\t<clip:200><a:gamelink\t%5>%1</a></clip><rmargin:280><just:right>%2<rmargin:375><just:right>%3',
                    %col1Client.name, %col1ClientScore, %col1ClientTime,
...
function RabbitGame::showRabbitWaypointClient(%game, %clRabbit, %client)
{
    ...
    if (!isObject(%clRabbit) || !isObject(%clRabbit.player) ||
            !isObject(%clRabbit.player.holdingFlag))
        return;
    ...
    %client.waypointSchedule = %game.schedule(
        %game.waypointDuration, "hideRabbitWaypointClient", %clRabbit, %client);
}
...
function RabbitGame::showRabbitWaypoint(%game, %clRabbit)
{
    ...
    %game.waypointSchedule = %game.schedule(%game.waypointDuration,
        "hideRabbitWaypoint", %clRabbit);
}
...
function RabbitGame::hideRabbitWaypoint(%game, %clRabbit)
{
    ...
    if (isObject(%clRabbit.player) && isObject(%clRabbit.player.holdingFlag))
        %game.waypointSchedule = %game.schedule(
            %game.waypointFrequency, "showRabbitWaypoint", %clRabbit);
}

function RabbitGame::updateKillScores(%game, %clVictim, %clKiller,
        %damageType, %implement)
{
    // check for turretkill before awarded a non client points for a kill
