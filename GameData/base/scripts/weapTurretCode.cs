...
function Ammo::onCollision(%data, %obj, %col)
{
    ...
    if (%col.getDataBlock().className $= Armor)
    {
        ...
        if (%col.inv[%ammoName] < %aMax)
        {
            ...
            %obj.respawn();
            if (%col.client > 0)
                messageClient(%col.client, 'MsgItemPickup',
                    '\c0You picked up %1.', %data.pickUpName);
        }
    }
}
...
function HandInventory::onCollision(%data, %obj, %col)
{
    ...
    if (%col.getDataBlock().className $= Armor)
    {
        ...
        if (%data.isGrenade)
        {
            // it's a grenade -- see if it matches the type the player is
            // carrying
            %pgType = "None";
            ...
            {
                // player either has no grenades or this type of grenades -- OK
                // to pick up more
                %canPickup = true;
            }
            else
            {
                // player has a different kind of grenade -- don't pick this
                // kind up
                %canPickup = false;
            }
        }
        ...
        if (%canPickup)
        {
            ...
            {
                //--------------------------------------------------------------
                // z0dd - ZOD, 5/8/02. Don't allow player to pickup full ammo if
                // they tossed less than full.
...
function BomberTurret::damageObject(%this, %targetObject, %sourceObject,
        %position, %amount, %damageType, %vec, %client, %projectile)
{
    //If vehicle turret is hit then apply damage to the vehicle
    %vehicle = %targetObject.getObjectMount();
    if (%vehicle)
        %vehicle.getDataBlock().damageObject(%vehicle, %sourceObject, %position,
            %amount, %damageType, %vec, %client, %projectile);
}
...
function MobileTurretBase::damageObject(%this, %targetObject, %sourceObject,
        %position, %amount, %damageType, %vec, %client, %projectile)
{
    //If vehicle turret is hit then apply damage to the vehicle
    %vehicle = %targetObject.getObjectMount();
...
function AssaultPlasmaTurret::damageObject(%this, %targetObject, %sourceObject,
        %position, %amount, %damageType , %vec, %client, %projectile)
{
    //If vehicle turret is hit then apply damage to the vehicle
    %vehicle = %targetObject.getObjectMount();
...
function CameraGrenadeThrown::onStickyCollision(%data, %obj)
{
    ...
    if (%dcSucc == 0)
        messageClient(%obj.sourceObject.client, 'MsgDeployFailed',
            '\c2Your team\'s control network has reached its capacity for this item.~wfx/misc/misc.error.wav');
...
function detonateGrenade(%obj)
{
    ...
    RadiusExplosion(%obj, %obj.getPosition(), %data.damageRadius,
        %data.indirectDamage, %data.kickBackStrength, %obj.sourceObject,
        ...
        if (!%prevWhiteOut)
            if (!$teamDamage)
            {
                if (%damage.client != %thrower && %damage.client.team == %thrower.team)
                    messageClient(%damage.client, 'teamWhiteOut',
                        '\c1You were hit by %1\'s whiteout grenade.',
                        getTaggedString(%thrower.name));
            }
...
function deployMineCheck(%mineObj, %player)
{
    ...
    if (%mineObj.getVelocity() $= "0 0 0")
    {
        ...
        schedule(%mineObj.getDatablock().armTime, %mineObj,
            "armDeployedMine", %mineObj);

        // check for other deployed mines in the vicinity
        InitContainerRadiusSearch(%mineObj.getWorldBoxCenter(),
            %mineObj.getDatablock().spacing, $TypeMasks::ItemObjectType);
