...
function serverCmdListenTo(%client, %who, %boolean)
{
    ...
    {
        if (%boolean)
            messageClient(%client, 'MsgVoiceEnable',
                'You will now listen to %3.', %boolean, %who, %who.name);
        else
            messageClient(%client, 'MsgVoiceEnable',
                'You will no longer listen to %3.', %boolean, %who, %who.name);
    }
    else
...
function serverCmdSetVoiceInfo(%client, %channels, %decodingMask, %encodingLevel)
{
    ...
    commandToClient(%client, 'SetVoiceInfo',
        %channels, %decodingMask, %encodingLevel);
