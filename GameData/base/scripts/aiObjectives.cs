...
function AIObjectiveGetClosestClient(%location, %team)
{
    ...
    {
        %cl = ClientGroup.getObject(%i);
        if (%cl.isAIControlled() && (%cl.team == %team || %team == 0) &&
                AIClientIsAlive(%cl))
        {
...
function AIChooseObjective(%client, %useThisObjectiveQ)
{
    ...
    // since most objectives check for inventory, find the closest inventory
    // stations first
    ...
    {
        // should re-evaluate the current objective weight - but never decrease
        // the weight!!!
        %testWeight = %client.objective.weight(
            %client, %client.objectiveLevel, 0, %inventoryStr);
        if (%testWeight <= 0 || %testWeight > %client.objectiveWeight)
            %client.objectiveWeight = %testWeight;

        if (%client.objectiveWeight > 0)
            AIAddTableObjective(%client.objective, %client.objectiveWeight,
                %client.objectiveLevel, 0, 0);
        else
            AIAddTableObjective(0, 0, 0, 0, 0);
    }
    ...
    for (%i = 0; %i < %objCount; %i++)
    {
        ...
        for (%level = 1; %level <= 4; %level++)
        {
            ...
            %minWeight = (%bumpWeight > $objTable[0, weight] ?
                %bumpWeight : $objTable[0, weight]);
            ...
            // else if it's equal to the highest objective we've seen so far,
            // and higher from our current objective
            else if (%weight == $objTable[0, weight] && %weight > %client.objectiveWeight)
            {
                //never bump someone unless you outweigh them
                if (%weight > %bumpWeight)
                {
                    // if this wouldn't require us to bump someone, or the table
                    // is empty
                    if (%bumpWeight <= 0 || $objTable[0, weight] <= 0)
                    {
                        // if the table currently contains objectives which
                        // would require bumping someone, clear it
                        ...
                        //add it to the table
                        AIAddTableObjective(%objective, %weight, %level,
                            %bumpClient, %position);

                        // no need to keep checking the other levels
                        break;
                    }
                    // otherwise, the table is not empty, and this would require
                    // us to bump someone only add it if everything else in the
                    // table would also require us to bump someone
                    else if ($objTable[0, bump] > 0)
                    {
                        AIAddTableObjective(%objective, %weight, %level,
                            %bumpClient, $objTableCount);
            ...
            // else it must have been less than our highest objective so far-
            // again, no need to keep checking other levels...
            else
                break;
        }
    }

    // if we have a table of possible objectives which are higher than our
    // current- choose one at random
    if ($objTableCount > 0 && $objTable[0, objective] != %client.objective)
    {
        ...
        if (%chooseObjective.shouldAcknowledge &&
                %chooseObjective.issuedByHuman &&
                isObject(%chooseObjective.issuedByClientId))
        {
            // cancel any pending acknowledgments - a bot probably just got
            // bumped off this objective
            cancel(%chooseObjective.ackSchedule);
            %chooseObjective.ackSchedule = schedule(5500, %chooseObjective,
                "AIAcknowledgeObjective", %client, %chooseObjective);
        }
...
function AIAcknowledgeObjective(%client, %objective)
{
    ...
    if (%client.objective == %objective)
        serverCmdAcceptTask(%client, %objective.issuedByClientId, -1,
            %objective.ackDescription);
}
...
function AIDefendLocation::assume(%task, %client)
{
    ...
    // set a flag to determine if the objective should be re-acquired when the
    // object is destroyed
...
function AIDefendLocation::weight(%task, %client)
{
    ...
    %hasMissile = (%player.getInventory("MissileLauncher") > 0) &&
        (%player.getInventory("MissileLauncherAmmo") > 0);

	// if we're defending with a missile launcher, our first priority is to take
    // out vehicles... see if we're already attacking a vehicle...
...
function AIDefendLocation::monitor(%task, %client)
{
    ...
    if (%client.needEquipment)
    {
        ...
        %result = AIBuyInventory(%client, %equipmentList,
            %task.buyEquipmentSet, %task.buyInvTime);
        ...
        else if (getSimTime() - %task.sendMsgTime > 7000)
        {
            ...
            if (%client.isAIControlled())
            {
                if (%task.chat !$= "")
                {
                    ...
                    if (%chatTemplate !$= "")
                        AIMessageThreadTemplate(
                            %chatTemplate, %chatMsg, %client, -1);
                    else
                ...
                else if (%task.targetObject > 0)
                {
                    ...
                    if (%type $= "Flag")
                        AIMessageThreadTemplate("DefendBase",
                            "ChatSelfDefendFlag", %client, -1);
                    else if (%type $= "GeneratorLarge")
                        AIMessageThreadTemplate("DefendBase",
                            "ChatSelfDefendGenerator", %client, -1);
                    else if (%type $= "StationVehicle")
                        AIMessageThreadTemplate("DefendBase",
                            "ChatSelfDefendVehicle", %client, -1);
                    else if (%type $= "SensorLargePulse")
                        AIMessageThreadTemplate("DefendBase",
                            "ChatSelfDefendSensors", %client, -1);
                    else if (%type $= "SensorMediumPulse")
                        AIMessageThreadTemplate("DefendBase",
                            "ChatSelfDefendSensors", %client, -1);
                    else if (%type $= "TurretBaseLarge")
                        AIMessageThreadTemplate("DefendBase",
                            "ChatSelfDefendTurrets", %client, -1);
                }
            }
        }
    }
            ...
            if (%dist < 10)
            {
                ...
                if (%task == %client.objectiveTask)
                {
                    if (aiHumanHasControl(%task.issuedByClient, %client))
                    {
                        ...
                        %inventoryStr = AIFindClosestInventories(%client);
                        %client.objectiveWeight = %client.objective.weight(
                            %client, %client.objectiveLevel, 0, %inventoryStr);
                    }
                }
    ...
    if (!AIClientIsAlive(%client.getEngageTarget()) &&
            AIClientIsAlive(%client.shouldEngage))
...
function AIAttackLocation::assume(%task, %client)
{
    ...
    {
        //see if we could benefit from inventory
        %needArmor = AIMustUseRegularInvStation(
            %task.desiredEquipment, %client);
        %result = AIFindClosestInventory(%client, %needArmor);
        %closestInv = getWord(%result, 0);
        %closestDist = getWord(%result, 1);
        ...
        if (AINeedEquipment(%task.desiredEquipment, %client) && %closestInv > 0)
        {
            %result = AIFindClosestEnemyToLoc(
                %client, %task.location, 50, $AIClientLOSTimeout);
            %closestEnemy = getWord(%result, 0);
...
function AIAttackLocation::weight(%task, %client)
{
    ...
    if (%client.player.getInventory(SniperRifle) > 0 &&
            %client.player.getInventory(EnergyPack) > 0 && %distToLoc > 60)
...
function AIAttackLocation::monitor(%task, %client)
{
        ...
        else if (getSimTime() - %task.sendMsgTime > 7000)
        {
            ...
            if (%client.isAIControlled())
            {
                if (%task.chat !$= "")
                {
                    ...
                    AIMessageThreadTemplate(
                        "AttackBase", "ChatSelfAttack", %client, -1);
            }
        }
    }
    ...
    if (%client.player.getInventory(SniperRifle) > 0 &&
            %client.player.getInventory(EnergyPack) > 0)
    {
        // first, find an LOS location
        if (%task.snipeLocation $= "")
        {
            %task.snipeLocation = %client.getLOSLocation(
                %task.location, 150, 250);
            %task.hideLocation = %client.getHideLocation(
                %task.location, VectorDist(%task.location, %task.snipeLocation),
        ...
        else
        {
            ...
            // until we're in position, we can move using the AIModeExpress,
            // after that, we only want to walk...
            ...
            {
                %client.stepRangeObject(
                    %task.engageTarget.player.getWorldBoxCenter(),
                    "BasicSniperShot", 150, 250, %task.snipelocation);
...
function AIAttackPlayer::assume(%task, %client)
{
    ...
    if (!%client.needEquipment && %task.buyEquipmentSet !$= "")
    {
        ...
        if (AINeedEquipment(%task.desiredEquipment, %client) && %closestInv > 0)
        {
            %distToTarg = %client.getPathDistance(
                %task.targetClient.player.getWorldBoxCenter());
...
function AIAttackPlayer::monitor(%task, %client)
{
    ...
    if (%client.needEquipment)
    {
        ...
        if (%task.equipment !$= "")
            ...
            %equipmentList = %task.desiredEquipment;
        %result = AIBuyInventory(
            %client, %equipmentList, %task.buyEquipmentSet, %task.buyInvTime);
        if (%result $= "InProgress")
            return;
        else if (%result $= "Finished")
        {
...
function AITouchObject::assume(%task, %client)
{
    ...
    //even if we don't *need* equipment, see if we should buy some...
    if (!%client.needEquipment &&
            (%task.mode $= "FlagGrab" || %task.mode $= "TouchFlipFlop") &&
            %task.buyEquipmentSet !$= "")
    {
        //see if we could benefit from inventory
        %needArmor = AIMustUseRegularInvStation(%task.desiredEquipment, %client);
        %result = AIFindClosestInventory(%client, %needArmor);
...
function AITouchObject::monitor(%task, %client)
{
    ...
    if (%task.sendMsg)
    {
        ...
        else if (getSimTime() - %task.sendMsgTime > 7000)
        {
            ...
            {
                if (%task.mode $= "FlagGrab")
                    AIMessageThreadTemplate("AttackBase", "ChatSelfAttackFlag",
                        %client, -1);
            }
        }
    }
    ...
    if (%task.mode $= "FlagCapture")
    {
        ...
        // if we're within range of the flag's home position, and the flag isn't
        // home, start idling...
...
function AIEscortPlayer::assume(%task, %client)
{
    ...
    if (%task == %client.objectiveTask && %client == %task.forceClient &&
            %task.issuedByClient == %task.targetClient)
    {
        %client.needEquipment = false;
...
function AIEscortPlayer::weight(%task, %client)
{
    ...
    {
        if (AIClientIsAlive(%task.targetClient.lastDamageClient, %losTimeout) &&
                getSimTime() - %task.targetClient.lastDamageTime < %losTimeout)
    ...
    // if both us and the person we're escorting are in a vehicle, set the
    // weight high!
    ...
    for (%i = 0; %i < %targetCount; %i++)
    {
        ...
        {
            // find out which item is being targeted...
            %targPoint = %targ.getTargetPoint();
            InitContainerRadiusSearch(%targPoint, 10,
                $TypeMasks::TurretObjectType | $TypeMasks::StaticShapeObjectType);
...
function AIEscortPlayer::monitor(%task, %client)
{
    ...
    %hasMortar = (%client.player.getInventory("Mortar") > 0) &&
        (%client.player.getInventory("MortarAmmo") > 0);
    %hasMissile = (%client.player.getInventory("MissileLauncher") > 0) &&
        (%client.player.getInventory("MissileLauncherAmmo") > 0);
    if (!isObject(%task.engageTarget) && isobject(%task.missileTarget) &&
            %task.missileTarget.getDamageState() !$= "Destroyed" && (%hasMortar || %hasMissile))
    {
            ...
            if (%client.isAIControlled())
            {
                ...
                else if (%task.targetObject > 0)
                {
                    ...
                    if (%type $= "GeneratorLarge")
                        AIMessageThreadTemplate("AttackBase",
                            "ChatSelfAttackGenerator", %client, -1);
                    else if (%type $= "SensorLargePulse")
                        AIMessageThreadTemplate("AttackBase",
                            "ChatSelfAttackSensors", %client, -1);
                    else if (%type $= "SensorMediumPulse")
                        AIMessageThreadTemplate("AttackBase",
                            "ChatSelfAttackSensors", %client, -1);
                    else if (%type $= "TurretBaseLarge")
                        AIMessageThreadTemplate("AttackBase",
                            "ChatSelfAttackTurrets", %client, -1);
                    else if (%type $= "StationVehicle")
                        AIMessageThreadTemplate("AttackBase",
                            "ChatSelfAttackVehicle", %client, -1);
                }
            }
        }
    }
...
function AIRepairObject::initFromObjective(%task, %objective, %client)
{
    ...
    if (%task.deployed)
    {
        ...
        %task.deployDirection = MatrixMulVector("0 0 0 " @
            getWords(%objective.getTransform(), 3, 6), "0 1 0");
...
function AIRepairObject::assume(%task, %client)
{
    ...
    if (!%client.needEquipment)
    {
        if (%task.deployed)
        {
            %task.repairLocation = VectorAdd(%task.location,
                VectorScale(%task.deployDirection, -4.0));
            %client.stepMove(%task.repairLocation, 0.25);
        }
        else
            %client.stepRangeObject(
                %task.targetObject, "DefaultRepairBeam", 3, 8);
    }
...
function AIRepairObject::monitor(%task, %client)
{
    ...
    if (%client.needEquipment)
    {
        ...
        if (%client.player.getInventory(RepairPack) > 0)
        {
            ...
            // otherwise, we'll need to range it...
                ...
                %client.stepRangeObject(%task.targetObject,
                    "DefaultRepairBeam", 3, 8);
        }
        else
        {
            ...
            // search the AIItemSet for a repair pack (someone might have
            // dropped one...)
            ...
            // choose whether we're picking up the closest pack, or buying from
            // an inv station...
            if ((isObject(%closestRepairPack) &&
                    %closestRepairPack != %task.pickupRepairPack) ||
                    (%task.buyInvTime != %client.buyInvTime))
            {
                %task.pickupRepairPack = %closestRepairPack;

                // initialize the inv buying
                %task.buyInvTime = getSimTime();
                AIBuyInventory(%client, "RepairPack",
                    %task.buyEquipmentSet, %task.buyInvTime);
            ...
            {
                %result = AIBuyInventory(%client, "RepairPack",
                    %task.buyEquipmentSet, %task.buyInvTime);
                if (%result $= "InProgress")
                    return;
                else if (%result $= "Finished")
                {
                    ...
                    // if this is to repair a deployed object, walk to the
                    // deploy point...
                    if (%task.deployed)
                    {
                        %task.repairLocation = VectorAdd(%task.location,
            ...
            if (%client.isAIControlled())
            {
                if (%task.chat !$= "")
                {
                    ...
                    if (%chatTemplate !$= "")
                        ...
                        IMessageThread(%task.chat, %client, -1);
                }
                else if (%task.targetObject > 0)
                {
                    %type = %task.targetObject.getDataBlock().getName();
                    if (%type $= "GeneratorLarge")
                        AIMessageThreadTemplate("RepairBase",
                            "ChatSelfRepairGenerator", %client, -1);
                    else if (%type $= "StationVehicle")
                        AIMessageThreadTemplate("RepairBase",
                            "ChatSelfRepairVehicle", %client, -1);
                    else if (%type $= "SensorLargePulse")
                        AIMessageThreadTemplate("RepairBase",
                            "ChatSelfRepairSensors", %client, -1);
                    else if (%type $= "SensorMediumPulse")
                        AIMessageThreadTemplate("RepairBase",
                            "ChatSelfRepairSensors", %client, -1);
                    else if (%type $= "TurretBaseLarge")
                        AIMessageThreadTemplate("RepairBase",
                            "ChatSelfRepairTurrets", %client, -1);
                }
            }
        }
    }
    ...
    if (%task.targetObject.getDamagePercent() > 0)
    {
        ...
        if (%task.deployed)
        {
            ...
            %dist2D = VectorDist(%client.player.position,
                getWords(%task.repairLocation, 0, 1) SPC
                getWord(%client.player.position, 2));

            // set the aim when we get near the target...  this will be
            // overwritten when we're actually trying to deploy
...
function AILazeObject::assume(%task, %client)
{
    ...
    %client.setTargetObject(-1);
    if (!%client.needEquipment)
        %client.stepRangeObject(%task.targetObject, "BasicTargeter",
            80, 300, %task.issuedByClient.player.getWorldBoxCenter());
...
function AILazeObject::monitor(%task, %client)
{
    ...
    if (%client.needEquipment)
    {
        ...
        else if (%result $= "Finished")
        {
            ...
            %client.needEquipment = false;
            %client.stepRangeObject(
                %task.targetObject, "BasicTargeter", 80, 300);
        }
        else if (%result $= "Failed")
        {
            // if this task is the objective task, choose a new objective
    ...
    if (isObject(%task.targetObject) && %task.targetObject.getDamageState() !$= "Destroyed")
    {
        ...
        // look to see if anyone else is also targeting...
        %foundTarget = false;
        ...
        for (%i = 0; %i < %numTargets; %i++)
        {
            ...
            {
                %targDist = VectorDist(%targ.getTargetPoint(),
                    %task.targetObject.getWorldBoxCenter());
                if (%targDist < 10)
                {
                    %foundTarget = true;
                    break;
    ...
    else
    {
        ...
        if (%task.celebrate)
        {
            if (%task.waitTimerMS == 0)
            {
                ...
                schedule(%randTime, %client, "AIPlayAnimSound",
                    %client, %task.targetObject.getWorldBoxCenter(),
                    %sound, %minCel, %maxCel, 0);
...
function AIMortarObject::monitor(%task, %client)
{
    ...
    if (isObject(%task.targetObject) &&
            %task.targetObject.getDamageState() !$= "Destroyed")
    {
        %clientPos = %client.player.getWorldBoxCenter();
        ...
        // now start ask for someone to laze the target, and start a 20s timer
        else if (%task.waitForTargetter)
        {
            ...
            else
            {
                ...
                else if ((!%targetter || ! %targetter.isAIControlled()) &&
                        %elapsedTime > 20000)
                {
                    %task.waitForTargetter = false;
                    %task.waitTimerMS = 0;
        ...
        else
        {
            ...
            if (!%task.targetAcquired && AIClientIsAlive(%targetter) &&
                    %targetter.isAIControlled())
            {
                %client.setTargetObject(-1);
                %task.waitForTargetter = true;
            }
...
function AIDeployEquipment::initFromObjective(%task, %objective, %client)
{
    ...
    // use the Y-axis of the rotation as the desired direction of deployment,
...
function AIDeployEquipment::monitor(%task, %client)
{
    ...
    %task.deployFromLocation = VectorAdd(%task.location,
        VectorScale(%task.deployDirection, %factor));

    // see if we're within range of the deploy location
    %clLoc = %client.player.position;
    %distance = VectorDist(%clLoc, %task.deployFromLocation);
    %dist2D = VectorDist(%client.player.position,
        getWords(%task.deployFromLocation, 0, 1) SPC
        getWord(%client.player.position, 2));

    // set the aim when we get near the target... this will be overwritten when
    // we're actually trying to deploy
    if (%distance < 10 && %dist2D < 10)
        %client.aimAt(%task.location, 1000);
    ...
    if (%task.deployAttempts < 10 && %task.passes < 5 &&
            !AIClientIsAlive(%client.getEngageTarget()))
    {
        // dissolve the human control link
        if (%task == %client.objectiveTask)
            aiReleaseHumanControl(%client.controlByHuman, %client);
        ...
        if (%task.deployAttempts == 0)
            ...
            %deployPoint = findTurretDeployPoint(
                %client, %task.location, %task.deployAttempts);
        ...
        if (%deployPoint !$= "")
        {
            ...
            {
                // see if there's a "repairObject" objective for the newly
                // deployed thingy...
                if (%task == %client.objectiveTask)
                {
                    %deployedObject = %client.lastDeployedObject;

                    // search the current objective group and search for a
                    // "repair Object" task...
                    %objective = %client.objective;

                    // delete any previously associated "AIORepairObject"
                    // objective
                    ...
                    %objective.repairObjective = new AIObjective(AIORepairObject)
                    {
                        ...
                        weightLevel2 = 0;
                        description = "Repair the " @
                            %deployedObject.getDataBlock().getName();
                    ...
                    // finally, unassign the client so he'll go do something
                    // else...
                    AIUnassignClient(%client);
                    Game.AIChooseGameObjective(%client);
                }
    ...
    else if (!%task.checkObstructed)
    {
        ...
        InitContainerRadiusSearch(%task.location, 4,
            $TypeMasks::MoveableObjectType | $TypeMasks::VehicleObjectType |
    ...
    else if (%task.waitMove < 5 && %task.passes < 5)
    {
        ...
        if (%task.waitMove == 5)
        {
            ...
            %deployDist2D = VectorDist(getWords(%client.player.position, 0, 1)
                @ "0", getWords(%task.location, 0, 1) @ "0");
...
function AIODefault::weight(%objective, %client, %level, %inventoryStr)
{
    ...
    // if the objective doesn't require any equipment, it automatically
    // get's the +100...
    ...
    // if we need equipment, the distance is from the client, to an inv, then
    // to the target
    if (%needEquipment)
    {
        ...
        // find the closest inv based on whether we require armor (from a
        // regular inv station)
        ...
        // if we don't need armor, and there's no inventory station, see if the
        // equipment we need is something we can pick up off the ground (likely
        // this would be a repair pack...)
        if (%weightDist >= 32767)
        {
            ...
            for (%i = 0; %i < %itemCount; %i++)
            {
                ...
                if (%item.getDataBlock().getName() $= %itemType && !%item.isHidden())
                {
                    ...
                    if (%weightDist > 0)
                    {
                        // set the var so the distance function will work...
                        %weightInv = %item;
                        %found = true;
                        break;
                    }
...
function AIODefault::QuickWeight(%objective, %client, %level, %minWeight)
{
    ...
    // do a quick check to disqualify this objective if it can't meet the
    // minimum weight
    switch (%level)
    {
    case 1:
...
function AIODefendLocation::weight(%this, %client, %level, %minWeight, %inventoryStr)
{
    ...
    {
        if (!isObject(%this.targetObjectId) ||
                %this.targetObjectId.isHidden() ||
                %this.targetObjectId.team != %client.team)
        return 0;
    }
    ...
    if (%this.targetObjectId > 0)
    {
        ...
        else if (!isObject(%this.targetObjectId) ||
                %this.targetObjectId.getDamageState() $= "Destroyed")
            %weight -= 320;
    }
...
function AIOAttackLocation::weight(%this, %client, %level,
        %minWeight, %inventoryStr)
{
    // make sure the player is still alive!!!!!
    if (!AIClientIsAlive(%client))
        return 0;
...
function AIOAttackPlayer::weight(%this, %client, %level, %minWeight, %inventoryStr)
{
    ...
    // now, if this bot is linked to a human who has issued this command,
    // up the weight
    if (%this.issuedByClientId == %client.controlByHuman)
    {
        // make sure we have the potential to reach the minWeight
...
function AIOEscortPlayer::weight(%this, %client, %level,
        %minWeight, %inventoryStr)
{
    // make sure the player is still alive!!!!!
...
function AIORepairObject::weight(%this, %client, %level, %minWeight, %inventoryStr)
{
    ...
    if (!isObject(%this.targetObjectId) ||
            %this.targetObjectId.getDamagePercent() <= 0)
        return 0;
    else
    {
        // if this bot is linked to a human who has issued this command,
        // up the weight
        if (%this.issuedByClientId == %client.controlByHuman)
        {
            // make sure we have the potential to reach the minWeight
...
function AIOLazeObject::weight(%this, %client, %level, %minWeight, %inventoryStr)
{
    ...
    for (%i = 0; %i < %numTargets; %i++)
    {
        ...
        {
            %targDist = VectorDist(%targ.getTargetPoint(),
                %this.targetObjectId.getWorldBoxCenter());
            if (%targDist < 10)
            {
    ...
    if (!isObject(%this.targetObjectId) ||
            %this.targetObjectId.getDamageState() $= "Destroyed")
        return 0;
    else if (%this.targetObjectId.isHidden() ||
            %this.targetObjectId.team != %client.team)
        return 0;
    else if (getSimTime() - %this.lastLazedTime <= 15000 &&
            %this.lastLazedClient != %client)
        return 0;
    else
    {
        // set the base weight
        ...
        // for now, do not deviate from the current assignment to laze a target,
        // if you don't already have a targeting laser.
        ...
        if (%this.issuedByClientId > 0)
        {
            ...
            %distance = %client.getPathDistance(
                %this.issuedByClientId.player.getWorldBoxCenter());
            if (%distance < 0)
                %distance = 32767;
...
//------------------------------------------------------------------------
// If the function ShapeBaseImageData::testInvalidDeployConditions() changes at
// all, those changes need to be reflected here
function AIODeployEquipment::weight(%this, %client, %level, %minWeight, %inventoryStr)
{
    ...
    if (%this.equipment $= "TurretOutdoorDeployable" ||
            %this.equipment $= "TurretIndoorDeployable")
    ...
    InitContainerRadiusSearch(%this.location, $MinDeployableDistance,
    ...
    // make sure we're not invalidating the deploy location with the client's
    // own player object
    ...
    if (%this.equipment $= "TurretIndoorDeployable")
    {
        ...
        InitContainerRadiusSearch(%this.location, $TurretIndoorSpaceRadius,
            $TypeMasks::StaticShapeObjectType);
        ...
        %highestDensity = 0;
        InitContainerRadiusSearch(%this.location, $TurretIndoorSphereRadius,
            $TypeMasks::StaticShapeObjectType);
        %found = containerSearchNext();
    ...
    {
        // check if there's another turret close to the deploy location
        InitContainerRadiusSearch(%this.location, $TurretOutdoorSpaceRadius,
            $TypeMasks::StaticShapeObjectType);
        %found = containerSearchNext();
        if (isObject(%found))
        {
        ...
        // now see if there are too many turrets in the area...
        %highestDensity = 0;
        InitContainerRadiusSearch(%this.location, $TurretOutdoorSphereRadius,
            $TypeMasks::StaticShapeObjectType);
        %found = containerSearchNext();
        while (isObject(%found))
        {
    ...
    // if don't need equipment, see if we've past the "point of no return",
    // and should continue regardless
    if (!%needEquipment)
    {
        ...
        // if we're too far from the inv to go back, or we're too close to the
        // deploy location, force continue
        if (%closestDist > 50 && VectorDist(
                %client.player.getWorldBoxCenter(), %task.location) < 50)
        {
            %weight = AIODefault::weight(%this, %client, %level, %inventoryStr);
