...
function AIHuntersTask::weight(%task, %client)
{
	...
	if (%client.flagCount < 5)
		...
		%damageTolerance = 0.25 +
            ((%client.getSkillLevel() * %client.getSkillLevel()) * 0.35);
	if (%damage > %damageTolerance)
	{
	   if (!isObject(%task.getHealth))
	   {
		   ...
		   {
                %healthDist = %client.getPathDistance(
                    %closestHealth.getWorldBoxCenter());
                if (%healthDist < 0)
                    %healthDist = 32767;
                else
        ...
        {
            %healthDist = %client.getPathDistance(
                %task.getHealth.getWorldBoxCenter());
            if (%healthDist < 0)
            {
                %healthDist = 32767;
                %task.getHealth = -1;
    ...
    if ((!Game.greedMode || %numToScore >= Game.greedMinFlags) &&
            !Game.hoardModeActive() && %numToScore >= $AIHuntersMinFlagsToCap)
    {
        ...
        if (AIClientIsAlive(%currentTarget))
        {
            ...
            %closestEnemydist = %client.getPathDistance(
        ...
        if ((AICheckEnemyDanger(%client, 35) >= 3 &&
                %damage > %damageTolerance) || (!isObject(%closestFlag) ||
                %closestFlagDist > $AIHuntersCloseFlagDist))
        {
            ...
            else if (AIClientIsAlive(%closestEnemy) &&
                    AIEngageWhoWillWin(%closestEnemy, %client) == %closestEnemy)
                %mustCap = true;
            ...
            else
            {
                ...
                if (%needFlagsForLead == 0 || (%needFlagsForLead > %waitForFlagsTolerance))
                {
                    ...
                    if (%needFlagsForLead == 0 ||
                            %numEnemyFlags < %needFlagsForLead || Game.teamMode)
                    {
                        if (%numToScore >= $AIHuntersMinFlagsToCap +
                                (%client.getSkillLevel() * %client.getSkillLevel() * 15))
                        {
                            %shouldCap = true;
                        }
                    }
    ...
    if (isObject(%closestFlag))
    {
        ...
        if (AIClientIsAlive(%bestClientToEngage))
        {
            //calc weight base on closeness to the nearest flag vs. and number
            //of flags the client has...
...
function AIHuntersTask::monitor(%task, %client)
{
    ...
    //if we're not engaging someone related to the hunters task, engage whoever
    //the AIEngageTask wants...
    else
...
function findClientWithMostFlags(%srcClient, %losTimeout)
{
    ...
    for (%i = 0; %i < %count; %i++)
    {
        ...
        {
            %clIsCloaked = false;
            if (%cl.player.getInventory("CloakingPack") > 0 &&
                    %cl.player.getImageState($BackpackSlot) $= "activate")
                %clIsCloaked = true;
            ...
            if (%hasLOS || (%losTime < %losTimeout &&
                    AIClientIsAlive(%cl, %losTime + 1000)))
            {
...
function aiHlist()
{
	...
	for (%i = 0; %i < %count; %i++)
	{
		...
		if (%cl.isAIControlled())
			error(%cl SPC getTaggedString(%cl.name) SPC "score:" SPC
                %cl.score SPC "flags:" SPC %cl.flagCount - 1 SPC
                "capFlags:" SPC %cl.huntersTask.capFlags);
	}
}
