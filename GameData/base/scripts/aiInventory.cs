...
function AIProcessBuyInventory(%client)
{
    ...
    if (!(%closestInv > 0 && isObject(%closestInv) &&
            (%closestInv.team <= 0 || %closestInv.team == %client.team) &&
            %closestInv.isEnabled()))
    {
        ...
        else if (%client.getStepStatus() $= "Finished")
        {
            ...
            else if (getSimTime() > %client.invWaitTime)
            {
                schedule(250, %client, "AIPlayAnimSound", %client,
                    %objSrch.getWorldBoxCenter(), "vqk.move", -1, -1, 0);
                %client.invWaitTime = getSimTime() + 5000 + (getRandom() * 10000);
            }
        }
        else
    ...
    else if (isObject(%closestInv) && isObject(%closestInv.trigger) &&
            VectorDist(%closestInv.trigger.getWorldBoxCenter(),
                %player.getWorldBoxCenter()) < 1.5)
    {
        //first stop...
        %client.stop();
    ...
    else
    {
        ...
        {
            //quite possibly we may need to deal with what happens if a bot
            //doesn't have a path to the inv...
            //the current premise is that no inventory stations are
            //"unpathable"...
...
function AIMustUseRegularInvStation(%equipmentList, %client)
{
    ...
    while (%item !$= "")
    {
        if (%item $= "InventoryDeployable" ||
                ...
                (%clientArmor $= "Light" &&
                    (%item $= "Mortar" || %item $= "MissileLauncher")))
        {
            return true;
        }
        else
...
function AICouldUseItem(%client, %item)
{
    ...
    //check packs first
    if (%type $= "RepairPack" || %type $= "EnergyPack" ||
            %type $= "ShieldPack" || %type $= "CloakingPack" || %type $= "AmmoPack")
    {
    ...
    //if the item is actually, a corpse, check the corpse inventory...
    if (%item.isCorpse)
    {
        %corpse = %item;
        if (%corpse.getInventory("ChainGunAmmo") > 0 &&
                %player.getInventory(%type) < %playerDataBlock.max[ChainGunAmmo])
            return true;
        if (%corpse.getInventory("PlasmaAmmo") > 0 &&
                %player.getInventory(%type) < %playerDataBlock.max[PlasmaAmmo])
            return true;
        if (%corpse.getInventory("DiscAmmo") > 0 &&
                %player.getInventory(%type) < %playerDataBlock.max[DiscAmmo])
            return true;
        if (%corpse.getInventory("GrenadeLauncher") > 0 &&
                %player.getInventory(%type) < %playerDataBlock.max[GrenadeLauncher])
            return true;
        if (%corpse.getInventory("MortarAmmo") > 0 &&
                %player.getInventory(%type) < %playerDataBlock.max[MortarAmmo] &&
                %player.getInventory("Mortar") > 0)
            return true;
    }
    else
    {
    ...
    //guess we didn't find anything useful...
    //(should still check for mines and grenades)
    return false;
}
...
function AIEngageOutofAmmo(%client)
{
    ...
    %hasSniper  = (%player.getInventory("SniperRifle") > 0) &&
        (%player.getInventory("EnergyPack") > 0);
...
function AICanPickupWeapon(%client, %weapon)
{
    ...
    if (%weapon !$= "Blaster" && %weapon !$= "Plasma" &&
            %weapon !$= "Chaingun" && %weapon !$= "Disc" &&
            %weapon !$= "GrenadeLauncher" && %weapon !$= "SniperRifle" &&
            %weapon !$= "ELFGun" && %weapon !$= "ShockLance")
        return false;
    ...
    if ((%armor $= "Light" && %weaponCount < 3) ||
            (%armor $= "Medium" && %weaponCount < 4) ||
            (%armor $= "Heavy" && %weaponCount < 5))
    {
        if ((%type $= "Mortar" && %armor !$= "Heavy") ||
                (%type $= "MissileLauncher" && %armor $= "Light") ||
...
function AIEngageWeaponRating(%client)
{
    ...
    %hasBlaster = (%player.getInventory("Blaster") > 0);
    %hasPlasma  = (%player.getInventory("Plasma") > 0 &&
        %player.getInventory("PlasmaAmmo") >= 1);
    %hasChain   = (%player.getInventory("Chaingun") > 0 &&
        %player.getInventory("ChaingunAmmo") >= 1);
    %hasDisc    = (%player.getInventory("Disc") > 0 &&
        %player.getInventory("DiscAmmo") >= 1);
    %hasGrenade = (%player.getInventory("GrenadeLauncher") > 0 &&
        %player.getInventory("GrenadeLauncherAmmo") >= 1);
    %hasSniper  = (%player.getInventory("SniperRifle") > 0) &&
        (%player.getInventory("EnergyPack") > 0);
    %hasELF     = (%player.getInventory("ELFGun") > 0);
    ...
    if (%hasDisc)
    {
        %quantity = %player.getInventory("DiscAmmo") /
            %playerDataBlock.max["DiscAmmo"];
        %rating += 15 + (15 * %quantity);
    }
    if (%hasPlasma)
    {
        %quantity = %player.getInventory("PlasmaAmmo") /
            %playerDataBlock.max["PlasmaAmmo"];
        %rating += 15 + (15 * %quantity);
    }
    if (%hasChain)
    {
        %quantity = %player.getInventory("ChainGunAmmo") /
            %playerDataBlock.max["ChainGunAmmo"];
        %rating += 15 + (15 * %quantity);
    }
    ...
    //note a rating of 20+ means at least two energy weapons, or an ammo weapon
    //with at least 1/3 ammo...
    return %rating;
}
...
function AIFindSafeItem(%client, %needType)
{
    ...
    for (%i = 0; %i < %itemCount; %i++)
    {
        ...
        %type = %item.getDataBlock().getName();
        if ((%needType $= "Health" &&
                (%type $= "RepairKit" || %type $= "RepairPatch") &&
                %player.getDamagePercent() > 0) || (%needType $= "Ammo" &&
                (%type $= "ChainGunAmmo" || %type $= "PlasmaAmmo" ||
                %type $= "DiscAmmo" || %type $= "GrenadeLauncherAmmo" ||
                %type $= "MortarAmmo") && AICouldUseItem(%client, %item)) ||
                (%needType $= "Ammo" && AICanPickupWeapon(%type)) ||
                ((%needType $= "" || %needType $= "Any") &&
                AICouldUseItem(%client, %item)))
        {
            ...
            if (%distance > 0 && %distance < %closestDist)
            {
                ...
                for (%j = 0; %j < %clientCount; %j++)
                {
                    ...
                    if (%cl == %client || %cl.team == %client.team ||
                            !AIClientIsAlive(%cl))
                        continue;
                    ...
                    {
                        %tempDist = %client.getPathDistance(
                            %item.getWorldBoxCenter());
...
function AIChooseObjectWeapon(%client, %targetObject, %distToTarg, %mode,
        %canUseEnergyStr, %environmentStr)
{
    //get our inventory
    %player = %client.player;
	if (!isObject(%player))
    ...
    %hasPlasma = (%player.getInventory("Plasma") > 0) &&
        (%player.getInventory("PlasmaAmmo") > 0) && !%inWater;
    %hasChain = (%player.getInventory("Chaingun") > 0) &&
        (%player.getInventory("ChaingunAmmo") > 0);
    %hasDisc = (%player.getInventory("Disc") > 0) &&
        (%player.getInventory("DiscAmmo") > 0);
    %hasGrenade = (%player.getInventory("GrenadeLauncher") > 0) &&
        (%player.getInventory("GrenadeLauncherAmmo") > 0);
    %hasMortar = (%player.getInventory("Mortar") > 0) &&
        (%player.getInventory("MortarAmmo") > 0);
    ...
    if (%mode $= "Destroy")
    {
        if ((%targetObject.getDataBlock().getClassName() $= "TurretData" ||
                %targetObject.getDataBlock().getName() $= "MineDeployed") &&
                %distToTarg < 50)
        {
            if (%hasPlasma)
                %useWeapon = "Plasma";
...
function AIChooseEngageWeapon(%client, %targetClient, %distToTarg,
        %canUseEnergyStr, %environmentStr)
{
	//get some status
    %player = %client.player;
    ...
    %hasGrenade = (%player.getInventory("GrenadeLauncher") > 0) &&
        (%player.getInventory("GrenadeLauncherAmmo") > 0);
    %hasSniper = (%player.getInventory("SniperRifle") > 0) &&
        (%player.getInventory("EnergyPack") > 0) && %canUseEnergy && !%inWater;
    %hasELF = (%player.getInventory("ELFGun") > 0) && %canUseEnergy && !%inWater;
    %hasMortar = (%player.getInventory("Mortar") > 0) &&
        (%player.getInventory("MortarAmmo") > 0);
    %hasMissile = (%player.getInventory("MissileLauncher") > 0) &&
        (%player.getInventory("MissileLauncherAmmo") > 0);
    %hasShockLance = (%player.getInventory("ShockLance") > 0) &&
        %canUseEnergy && !%inWater;
    ...
    {
        if (%hasELF && %myEnergy > 0.5 && %targEnergy > 0.3 &&
                %myDamage < %targDamage && %targetZ > %myZ + 20)
            %useWeapon = "ELFGun";
        else if (%hasPlasma)
            %useWeapon = "Plasma";
        else if (%hasChain)
    ...
    {
        if (%hasELF && %targEnergy > 0.4 &&
                (%myDamage < 0.3 || %myDamage - %targDamage < 0.2) &&
                %targetZ > %myZ + 20)
            %useWeapon = "ELFGun";
...
//function is called once per frame, to handle packs, healthkits, grenades,
//etc...
function AIProcessEngagement(%client, %target, %type, %projectile)
{
	...
	if (%client.getSkillLevel() > 0.1 && %player.getDamagePercent() > 0.3 &&
            %player.getInventory("RepairKit") > 0)
	{
		...
		%skillValue = (1.0 - %client.getSkillLevel()) *
            (1.0 - %client.getSkillLevel());
    ...
    case "player":
        ...
        {
            //if the target is in range, and within 10-40 m, and heading in this
            //direction, toss a grenade
            if (!$AIDisableGrenades && %client.getSkillLevel() >= 0.3)
            {
                ...
                if (%grenadeType !$= "" && %client.targetInSight())
                {
                    ...
                    if (getWord(%clientPos, 2) - getWord(%targPos, 2) < 3)
                    {
                        ...
                        %direction = VectorDot(VectorSub(%clientPos, %targPos),
                            %target.player.getVelocity());
                        %facing = VectorDot(
                            VectorSub(%client.getAimLocation(), %clientPos),
                            VectorSub(%targPos, %clientPos));
                        if (%dist > 20 && %dist < 45 &&
                                (%direction > 0.9 || %direction < -0.9) &&
                                (%facing > 0.9))
                        {
...
function AIFindClosestInventory(%client, %armorChange)
{
    ...
    if (!%armorChange && %depCount > 0)
    {
        for (%i = 0; %i < %depCount; %i++)
        {
            ...
            if (%obj.getDataBlock().getName() $= "DeployedStationInventory" &&
                    %obj.team == %client.team && %obj.isEnabled())
            {
...
function AIFindClosestInventories(%client)
{
	...
	//if the regular inv station is closer than the deployed, don't bother with
    //the remote
	if (%closestDist < %closestRemoteDist)
		%returnStr = %closestInv SPC %closestDist;
	else
		%returnStr = %closestInv SPC %closestDist SPC
            %closestRemoteInv SPC %closestRemoteDist;
