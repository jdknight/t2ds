...
{
    // items will take 1 second (1000 milliseconds) to fade out
    %itemFadeTime = 1000;
...
function Beacon::onUse(%data, %obj)
{
    ...
    // extract the position of the player's camera from the eye transform
    // (first 3 words)
    ...
    else
    {
        ...
        {
            // if there's already a beacon where player is aiming, switch its
            // type otherwise, player can't deploy a beacon there
...
function switchBeaconType(%beacon)
{
    if (%beacon.getBeaconType() $= "friend")
    {
        ...
        {
            messageClient(%beacon.sourceObject.client, 'MsgDeployFailed',
                '\c2Your team\'s control network has reached its capacity for this item.~wfx/misc/misc.error.wav');
            return 0;
        }
