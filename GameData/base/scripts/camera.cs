...
function Observer::onTrigger(%data, %obj, %trigger, %state)
{
    ...
    case "justJoined":
        ...
        else if (%trigger == 2)
        {
            ...
            messageClient(%client.observeClient, 'Observer',
    ...
    case "observerFly":
        ...
        if (%trigger == 0)
        {
            if (!$Host::TournamentMode && $MatchStarted)
            {
                ...
                if (%client.lastTeam !$= "" && %client.lastTeam != 0
                        && Game.numTeams > 1)
                {
    ...
    case "observerFollow":
        ...
        if (%trigger == 0)
        {
            ...
            if (%nextClient > 0 && %nextClient != %client.observeClient)
            {
                ...
                {
                    %obj.setOrbitMode(
                        %nextClient.player, %transform, 0.5, 4.5, 4.5);
                ...
                else
                {
                    ...
                    %obj.setOrbitMode(%mount, %mount.getTransform(),
                        getWord(%params, 0), getWord(%params, 1),
                        getWord(%params, 2));
                    %client.observeClient = %nextClient;
                }
                ...
                messageClient(%nextClient, 'Observer',
                    '\c1%1 is now observing you.', %client.name);
        ...
        else if (%trigger == 3)
        {
            ...
            if (%prevClient > 0 && %prevClient != %client.observeClient)
            {
                ...
                {
                    %obj.setOrbitMode(
                        %prevClient.player, %transform, 0.5, 4.5, 4.5);
        ...
        else if (%trigger == 2)
        {
            ...
            messageClient(%client.observeClient, 'ObserverEnd',
                '\c1%1 is no longer observing you.', %client.name);
        }
...
function serverCmdObserveClient(%client, %target)
{
    ...
    %client.camera.getDataBlock().setMode(
        %client.camera, "observerFollow", %target.player);
    %client.setControlObject(%client.camera);
...
function updateObserverFlyHud(%client)
{
    ...
    if (!isObject(%client) || %client.team != 0 ||
            %client.getControlObject() != %client.camera ||
    ...
    %srcEyeVector = MatrixMulVector("0 0 0 " @
        getWords(%srcEyeTransform, 3, 6), "0 1 0");
    ...
    // schedule the next...
    %client.obsHudSchedule = schedule(500,
        %client, updateObserverFlyHud, %client);
}
