...
function Armor::onMount(%this, %obj, %vehicle, %node)
{
    if (%node == 0)
    {
        ...
        if (!%obj.inStation)
            %obj.lastWeapon = (%obj.getMountedImage($WeaponSlot) == 0 ?
                "" : %obj.getMountedImage($WeaponSlot).getName().item);
        ...
        {
            schedule(15000, %obj.lastVehicle,
                "vehicleAbandonTimeOut", %obj.lastVehicle);
            %obj.lastVehicle.lastPilot = "";
        }
        if (%vehicle.lastPilot !$= "" &&
                %vehicle == %vehicle.lastPilot.lastVehicle)
        ...
        if (%vehicle.getTarget() != -1 &&
                %vehicle.getDatablock().cantTeamSwitch $= "")
        {
            setTargetSensorGroup(%vehicle.getTarget(),
                %obj.client.getSensorGroup());
            if (%vehicle.turretObject > 0)
                setTargetSensorGroup(%vehicle.turretObject.getTarget(),
                    %obj.client.getSensorGroup());
        }

        // Send a message to the client so they can decide if they want to
        // change view or not:
    ...
    if (%vehicle.getDatablock().numMountPoints > 1)
    {
        ...
        for (%i = 0; %i < %vehicle.getDatablock().numMountPoints; %i++)
        {
            if (%vehicle.getMountNodeObject(%i) > 0)
            {
                ...
                {
                    %team = (%obj.team == %vehicle.getMountNodeObject(%i).client.team ?
                        'Teammate' : 'Enemy');
                    messageClient(%vehicle.getMountNodeObject(%i).client,
                        'MsgShowPassenger', '\c2%3: \c3%1\c2 has boarded in the \c3%2\c2 position.',
                        %obj.client.name, %nodeName, %team);
                }
        ...
        for (%i = 0; %i < %vehicle.getDatablock().numMountPoints; %i++)
        {
            if (%vehicle.getMountNodeObject(%i) > 0)
            {
                if (%vehicle.getMountNodeObject(%i).client != %obj.client)
                {
                    ...
                    messageClient(%vehicle.getMountNodeObject(%i).client,
                        'MsgShowPassenger',
                        '\c2%3: \c3%1\c2 has ejected from the \c3%2\c2 position.',
                        %obj.client.name, %nodeName, %team);
                }

                commandToClient(%vehicle.getMountNodeObject(%i).client,
                    'showPassenger', %node, false);
            }
        }
    }
...
function Armor::onCollision(%this, %obj, %col, %forceVehicleNode)
{
    ...
    %node = -1;
    if (%forceVehicleNode !$= "" || %className $= WheeledVehicleData ||
            %className $= FlyingVehicleData || %className $= HoverVehicleData &&
            %obj.mountVehicle && %obj.getState() $= "Move" && %col.mountable &&
            !%obj.inStation && %col.getDamageState() !$= "Destroyed")
    {
        // if the player is an AI, he should snap to the mount points in node
        // order, to ensure they mount the turret before the passenger seat,
        // regardless of where they collide...
        if (%obj.client.isAIControlled())
        {
            %transform = %col.getTransform();

            // either the AI is *required* to pilot, or they'll pick the first
        ...
        if (%node >= 0)
        {
            ...
            if (hasLargePack(%obj))
            {
                ...
                {
                    // send the player a message -- can't sit here with large
                    // pack
                    if (!%obj.noSitMessage)
                    {
                        %obj.noSitMessage = true;
                        %obj.schedule(2000, "resetSitMessage");
                        messageClient(%obj.client, 'MsgCantSitHere',
                            '\c2Pack too large, can\'t occupy this seat.~wfx/misc/misc.error.wav');
                    }
            ...
            if (!%obj.inStation)
                %col.lastWeapon = (%col.getMountedImage($WeaponSlot) == 0 ?
                    "" : %col.getMountedImage($WeaponSlot).getName().item);
            else
    ...
    else if (%className $= "Armor")
    {
        ...
        if (%col.getState() $= "Dead")
        {
            ...
            for (%i = 0; %obj.weaponCount < %obj.getDatablock().maxWeapons &&
                    $InvWeapon[%i] !$= ""; %i++)
            {
            ...
            if (%col.hasInventory("TargetingLaser"))
            {
                if (%obj.incInventory("TargetingLaser", 1) > 0)
                {
                    ...
                    %gotSomething = true;
                    messageClient(%obj.client, 'MsgItemPickup',
                        '\c0You picked up a targeting laser.');
                }
            }
            ...
            for (%j = 0; $ammoType[%j] !$= ""; %j++)
            {
                ...
                if (%ammoAmt)
                {
                    // incInventory returns the amount of stuff successfully
                    // grabbed
                    ...
                    if (%grabAmt > 0)
                    {
                        ...
                        %gotSomething = true;
                        messageClient(%obj.client, 'MsgItemPickup',
                            '\c0You picked up %1.', $ammoType[%j].pickUpName);
                        %obj.client.setWeaponsHudAmmo($ammoType[%j],
                            %obj.getInventory($ammoType[%j]));
                    }
                }
            }
                ...
                if (%corpseGrenAmt)
                {
                    ...
                    if (%playerGrenType $= "None" || %playerGrenType $= %gren)
                    {
                        ...
                        if (%taken > 0)
                        {
                            ...
                            %obj.client.setInventoryHudAmount(
                                %gren, %obj.getInventory(%gren));
                        }
                    }
                    break;
                }
                ...
                if (%mineAmt)
                {
                    if (%playerMineType $= "None" || %playerMineType $= %mine)
                    {
                        ...
                        if (%grabbed > 0)
                        {
                            ...
                            %gotSomething = true;
                            messageClient(%obj.client, 'MsgItemPickup',
                                '\c0You picked up %1.', %mine.pickUpName);
                            %obj.client.setInventoryHudAmount(
                                %mine, %obj.getInventory(%mine));
                        }
                    }
                    break;
                }
            ...
            if (%beacAmt)
            {
                ...
                if (%bTaken > 0)
                {
                    ...
                    %gotSomething = true;
                    messageClient(%obj.client, 'MsgItemPickup',
                        '\c0You picked up %1.', Beacon.pickUpName);
                    %obj.client.setInventoryHudAmount(
                        Beacon, %obj.getInventory(Beacon));
                }
            }
            ...
            if (%rkAmt)
            {
                ...
                if (%rkTaken > 0)
                {
                    ...
                    %gotSomething = true;
                    messageClient(%obj.client, 'MsgItemPickup',
                        '\c0You picked up %1.', RepairKit.pickUpName);
                    %obj.client.setInventoryHudAmount(
                        RepairKit, %obj.getInventory(RepairKit));
                }
            }
        }
...
function Armor::doDismount(%this, %obj, %forced)
{
    ...
    {
        %vec[0] = %obj.getObjectMount().getDatablock().getDismountOverride(
            %obj.getObjectMount(), %obj);
        %vec[0] = MatrixMulVector(
            %obj.getObjectMount().getTransform(), %vec[0]);
    }
    else
    {
...
function resetObserveFollow(%client, %dismount)
{
    if (%dismount)
    {
        ...
        {
            %client.observers[%i].camera.setOrbitMode(
                %client.player, %client.player.getTransform(), 0.5, 4.5, 4.5);
        }
    }
    else
    {
        ...
        for (%i = 0; %i < %client.observeCount; %i++)
        {
            %client.observers[%i].camera.setOrbitMode(%mount,
                %mount.getTransform(), getWord(%params, 0),
                getWord(%params, 1), getWord(%params, 2));
        }
    }
}
        ...
        %amount, %damageType, %momVec, %mineSC)
{
    ...
    if (%targetObject.isMounted() && %targetObject.scriptKilled $= "")
    {
        ...
        if (%mount.team == %targetObject.team)
        {
            ...
            if (%found != -1)
            {
                ...
                {
                    %mount.getDataBlock().damageObject(%mount, %sourceObject,
    ...
    // if the src object is a player object, player's don't have sensor groups
    ...
    //--------------------------------------------------------------------------
    // z0dd - ZOD, 5/8/02. Check to see if this turret has a valid owner, if not
    // clear the owner variable.
    ...
    else if (isObject(%sourceObject) && %sourceObject.getClassName() $= "Turret")
    {
        ...
        if (%sourceObject.owner !$= "" && (
                %sourceObject.owner.team != %sourceObject.team ||
                !isObject(%sourceObject.owner)))
            %sourceObject.owner = "";
    }
    // End z0dd - ZOD
    //--------------------------------------------------------------------------
    else if (isObject(%sourceObject) &&
            (%sourceObject.getClassName() $= "FlyingVehicle" ||
                %sourceObject.getClassName() $= "WheeledVehicle" ||
                %sourceObject.getClassName() $= "HoverVehicle"))
        %sourceTeam = getTargetSensorGroup(%sourceObject.getTarget());
    else
    {
    ...
    if (!$teamDamage && %targetClient != %sourceClient &&
            %targetTeam == %sourceTeam)
        return;

    if (%targetObject.isShielded && %damageType != $DamageType::Blaster)
        %amount = %data.checkShields(%targetObject, %position, %amount,
            %damageType);
    ...
    Game.onClientDamaged(
        %targetClient, %sourceClient, %damageType, %sourceObject);
    ...
    if (%targetObject.getState() $= "Dead")
    {
        ...
        {
            // only if <= 35 percent damage remaining
            if (%previousDamage >= 0.35)
            {
        ...
        %damLoc = %targetObject.getDamageLocation(%position);
        Game.onClientKilled(%targetClient, %sourceClient, %damageType,
            %sourceObject, %damLoc);
    }
    else if (%amount > 0.1)
    {
        if (%targetObject.station $= "" && %targetObject.isCloaked())
        {
            ...
            %targetObject.reCloak = %targetObject.schedule(
                500, "setCloaked", true);
        }
...
{
    %data.damageObject(%playerObject, 0,
        VectorAdd(%playerObject.getPosition(), %vec),
        %vecLen * %data.speedDamageScale, $DamageType::Ground);
}

function Armor::applyConcussion(%this, %dist, %radius,
...
function Player::pickup(%this, %obj, %amount)
{
    ...
    if (%data.className $= Pack && %this.getMountedImage($BackpackSlot) != 0)
        return 0;
...
function Player::use(%this, %data)
{
    ...
    if (%data $= "Backpack")
    {
        ...
        if (%this.isPilot())
        {
            messageClient(%this.client, 'MsgCantUsePack',
                '\c2You can\'t use your pack while piloting.~wfx/misc/misc.error.wav');
            return false;
        }
        else if (%this.isWeaponOperator())
        {
            messageClient(%this.client, 'MsgCantUsePack',
                '\c2You can\'t use your pack while in a weaponry position.~wfx/misc/misc.error.wav');
            return false;
        }
...
function Player::liquidDamage(%obj, %data, %damageAmount, %damageType)
{
    if (%obj.getState() !$= "Dead")
    {
        ...
        %obj.lDamageSchedule = %obj.schedule(50, "liquidDamage", %data,
...
function playDeathAnimation(%player, %damageLocation, %type)
{
    ...
    if (%type == $DamageType::Explosion || %type == $DamageType::Mortar ||
